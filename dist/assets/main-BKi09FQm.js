import { _ as Le, __tla as __tla_0 } from "./index-vDvYU5Qu.js";
...
Promise.all([
    ...
    const dt = new fs(new u(-.5, -.5, -.5), new u(.5, .5, .5)), me = new Ee, ye = new Ls;
    function be(r) {
        if (r?.isObject3D) {
    ...
    function pt({ scene: r, maxParticles: s = 800, gravity: t = -7.8, drag: i = 2, texture: e = null, pointSize: a = .66, blending: o = Ae } = {}) {
        ...
        function n() {
            ...
            x.width = x.height = 64;
            const T = x.getContext("2d"), B = T.createRadialGradient(64 / 2, 64 / 2, 0, 64 / 2, 64 / 2, 64 / 2);
            B.addColorStop(0, "rgba(255,255,255,1)"), B.addColorStop(1, "rgba(255,255,255,0)"), T.fillStyle = B, T.fillRect(0, 0, 64, 64);
            const L = new se(x);
        ...
        P.userData.persistent = !0, P.frustumCulled = !1, P.position.set(0, -20, 0), r.add(P);
        let y = 0;
        function _() {
            for(let b = 0; b < s; b++){
                const x = (y + b) % s;
        ...
        function R(b, x, T, B, L) {
            const U = x * 3;
            b[U] = T, b[U + 1] = B, b[U + 2] = L;
        }
        return {
            trigger (b, x = 1, T = {}) {
                const { count: B = 42, spread: L = .35, up: U = 3, horiz: ms = 2.2, ttl: C = [
                    .35,
                    .8
                ], sizeJitter: j = .5 } = T, $ = Math.max(1, Math.floor(B * x));
                for(let ys = 0; ys < $; ys++){
                    const S = _();
                    if (S === -1) break;
                    const z = Math.sqrt(Math.random()) * L, A = Math.random() * Math.PI * 2, K = z * Math.cos(A), ls = z * Math.sin(A), Q = Math.sqrt(Math.random()), X = Math.cos(A) * ms * Q * (.6 + .4 * Math.random()), ss = Math.sin(A) * ms * Q * (.6 + .4 * Math.random()), es = U * (.6 + .4 * Math.random()), O = C[0] + Math.random() * (C[1] - C[0]), W = (1 - j / 2 + Math.random() * j) * 1;
            ...
            update (b) {
                if (b <= 0) return;
                const x = t, T = Math.max(0, i);
                let B = !1;
                for(let C = 0; C < s; C++){
                    if (!p[C]) continue;
                    if (B = !0, d[C] += b, d[C] >= m[C]) {
                        p[C] = 0;
                        const A = C * 3;
                    ...
                    const j = C * 3;
                    c[j + 1] += x * b;
                    const $ = c[j], ys = c[j + 1], S = c[j + 2], z = Math.max(0, 1 - T * b);
                    c[j] = $ * z, c[j + 1] = ys * z, c[j + 2] = S * z, h[j] += c[j] * b, h[j + 1] += c[j + 1] * b, h[j + 2] += c[j + 2] * b;
                }
                B && (g.attributes.position.needsUpdate = !0);
                let L = 0, U = 0;
                for(let C = 0; C < s; C++)p[C] && (L++, U += 1 - d[C] / m[C]);
                const ms = L ? .25 + .75 * (U / L) : 1;
                v.size = a * ms;
            },
            get object3D () {
                return P;
            ...
            dispose () {
                r.remove(P), g.dispose(), v.dispose(), e || l.dispose();
            }
        };
    }
    function ct({ scene: r, size: s = 1.5, ttl: t = .9 } = {}) {
        ...
        function m(f, p) {
            ...
            const g = n / t, v = s * (1 + 1.6 * g);
            o.scale.setScalar(v), a.opacity = 1 - g;
        }
        function d() {
    ...
    class bt {
        constructor(s, t, i, e, a, o, n){
            this.dataClass = s, this.scene = t, this.audioClass = i, this.levelClass = e, this.paramsClass = a, this.camera = o, this.gameClass = n, this.playerHeight = .9, this.playerWidth = .5, this.player = new vs(new is(this.playerWidth, this.playerHeight, this.playerWidth), new js({
                color: 16711680,
                transparent: !0,
                opacity: 0
        ...
        playerMove() {
            ...
            if ((this.player.userData.body.linvel().x != 0 || this.player.userData.body.linvel().y != 0) && V(this.player, this.levelClass.boostHatMeshes) && !this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(V(this.player, this.levelClass.boostHatMeshes))].userData.fly && this.levelClass.boostHatMeshes.indexOf(V(this.player, this.levelClass.boostHatMeshes)) != this.player.userData.canFlyNum && (this.player.userData.canFly || (this.player.userData.canFly = !0, this.player.userData.canFlyJumps = this.player.userData.canFlyJumpsMax, this.player.userData.canFlyNum = this.levelClass.boostHatMeshes.indexOf(V(this.player, this.levelClass.boostHatMeshes)), this.levelClass.boostHatModels[this.player.userData.canFlyNum].userData.fly = !0, this.audioClass.takeAudio.isPlaying && this.audioClass.stopMusic([
                ...
                if (this.player.userData.splash = !1, this.levelClass.players.length < 2 ? (this.player.userData.live && (this.audioClass.pauseMusic([
                    ...
                    this.player.userData.playerAlive && (this.player.userData.readyJump = !1, this.player.userData.canFly = !1, this.player.userData.canFlyJumps = 0, this.player.userData.canFlyNum = null, this.player.userData.jumping = !1, this.player.userData.playerPowerJump = 1, this.player.userData.onGround = !1, this.player.userData.body.setLinvel({
                        ...
                        y: 0,
                        z: 0
                    }, !0), this.paramsClass.gameDir == "vert" ? this.player.userData.body.setTranslation(new u(this.player.userData.deadPos.x + (.1 + Math.random() * .2), this.player.userData.deadPos.y, this.player.userData.deadPos.z)) : this.player.userData.body.setTranslation(new u(this.player.userData.deadPos.x + (.1 + Math.random() * .2), this.player.userData.deadPos.y + k(1.1, 3.1), this.player.userData.deadPos.z)), this.player.userData.deadPos = new u(0, 0, 0), this.player.userData.live = !0, this.player.userData.playerAlive = !1);
                }
                this.reLiveField();
            } else {
            ...
            if (this.player.userData.canFlyJumps) {
                ...
                const i = new u().setFromMatrixPosition(this.player.userData.head.matrixWorld), e = new Ls;
                this.player.userData.head.getWorldQuaternion(e);
                const a = new Ls().setFromEuler(new J(0, Math.PI / 2, 0)), o = e.clone().multiply(a), l = new u(.01, .14, .05).clone().applyQuaternion(o);
    ...
    class vt {
        constructor(s, t, i, e, a, o, n, l, h, c, m, d, f, p, g, v){
            this.scene = s, this.audioClass = t, this.physicsClass = i, this.renderer = e, this.camera = a, this.isMobile = o, this.paramsClass = n, this.worldClass = l, this.initMatch = h, this.gameClass = m, this.splash = d, this.ring = f, this.dataClass = c, this.scoreClass = p, this.menuClass = g, this.assetsManager = v, this.playersLoaded = !1, this.cameraSpeed = .01, this.levelsMode = !1, this.levelsNoFric = !1, this.allLevels = this.dataClass.allLevels, this.randomNoFric = .3, this.randomAnimateHor = .2, this.randomAnimateVert = .2, this.canShowAds = !0, this.boostHatModel, this.boostHatPropeller, this.boostHatMesh, this.boostHatModels = [], this.boostHatMeshes = [], this.boostHatCoords = [], this.angryBird, this.birdFlyingMark = 10, this.distanceToBird = 20, this.angryBirdModel, this.maxHeight = 0, this.birdYes = !0, this.canHorDie = !1, this.planeWidth = 4, this.planeHeight = 10, this.planeDepth = 1, this.minPlaneWidthTic = 1, this.fixedDistanceHor = {
                ...
                min: 3,
                max: 4
            }, this.count = 120, this._dayColor = new os(16777215), this._nightColor = new os(16771488), this.mksWidth = 100, this.mksHeight = 100, this.geometryPlane = new Hs(this.mksWidth, this.mksHeight), this.materialPlane = new ks({
                color: 0,
                side: Me
            ...
            const P = new Te, y = .01;
            P.moveTo(5 * y, 5 * y), P.bezierCurveTo(5 * y, 5 * y, 4 * y, 2 * y, 0 * y, 2 * y), P.bezierCurveTo(-6 * y, 2 * y, -6 * y, 7 * y, -6 * y, 7 * y), P.bezierCurveTo(-6 * y, 10 * y, -3 * y, 14 * y, 5 * y, 18 * y), P.bezierCurveTo(12 * y, 14 * y, 16 * y, 10 * y, 16 * y, 7 * y), P.bezierCurveTo(16 * y, 7 * y, 16 * y, 2 * y, 10 * y, 2 * y), P.bezierCurveTo(7 * y, 2 * y, 5 * y, 5 * y, 5 * y, 5 * y);
            const _ = {
                depth: .22,
                bevelEnabled: !1,
                curveSegments: 12,
  ...
  ` + b.fragmentShader.replace("#include <lights_fragment_begin>", `
    ...
    `);
            ...
            function q(b = 64) {
                const x = document.createElement("canvas");
                x.width = x.height = b;
                const T = x.getContext("2d"), B = T.createRadialGradient(b / 2, b / 2, 0, b / 2, b / 2, b / 2);
                B.addColorStop(0, "rgba(255, 235, 175, 1)"), B.addColorStop(1, "rgba(255, 235, 175, 0)"), T.fillStyle = B, T.fillRect(0, 0, b, b);
                const L = new se(x);
        ...
        async loadBarriers() {
            let s = new is(.5, .7, 1), t = new ks({
                color: 52224,
                transparent: !0,
                opacity: 0
            });
            this.angryBird = new vs(s, t), this.angryBird.userData.name = "bird", this.angryBird.userData.speed = k(8, 13) / 100, this.angryBird.userData.flying = !1, this.angryBird.position.y = -5, this.physicsClass.addPhysicsToObject(this.angryBird);
        }
        async createLevel(s) {
            if (this.levelsMode = s, this.maxHeight = 0, this.birdFlyingMark = 10, document.querySelector(".lev_hud span").textContent = s, await this.loadTexture(), await this.loadBarriers(), this.boostHatModel = this.assetsManager.boostHatModel, this.boostHatPropeller = this.assetsManager.boostHatPropeller, this.boostHatMesh = this.assetsManager.boostHatMesh, this.birdYes && (this.angryBirdModel = this.assetsManager.angryBirdModel, this.scene.add(this.angryBirdModel), ut(this.angryBirdModel, this.renderer, this.camera, this.scene)), document.querySelectorAll(".player_panel")[0].classList.add("hidden_screen"), document.querySelectorAll(".player_panel")[1].classList.add("hidden_screen"), document.querySelectorAll(".player_panel")[2].classList.add("hidden_screen"), this.players.forEach((t, i, e)=>{
                ...
                if (this.paramsClass.gameDir == "hor") {
                    for(let a = 0; a < this.count; a++){
                        let o = k(this.planeWidth, this.planeWidth - 1), n = t + o / 2 + k(this.fixedDistanceHor.min, this.fixedDistanceHor.max), l = k(-1.2, 1.2) - this.planeHeight / 1.5;
                    ...
                    this.birdYes = !1;
                    for(let a = 0; a < this.count; a++){
                        let o = k(this.bounds.rightX / this.minPlaneWidthTic, this.bounds.rightX / 5);
                        ...
                        let n = i + k(this.fixedDistanceVert.min, this.fixedDistanceVert.max);
                        if (this.objs.topPlanes.data[a].position.y = n - 1.3, this.objs.grassPlanes.data[a].position.y = n, this.objs.sensorPlanes.data[a].position.y = n - .3, this.objs.topPlanes.data[a].size.y = .3, this.objs.grassPlanes.data[a].size.y = .7, this.objs.sensorPlanes.data[a].size.y = .9, a > 0 ? (this.objs.topPlanes.data[a].size.x = o + .3, this.objs.grassPlanes.data[a].size.x = o + .3, this.objs.sensorPlanes.data[a].size.x = o + .7) : (this.objs.topPlanes.data[a].size.x = 10, this.objs.grassPlanes.data[a].size.x = 10, this.objs.sensorPlanes.data[a].size.x = 10), this.objs.lamps.data[a].position.x = this.objs.grassPlanes.data[a].position.x, this.objs.lamps.data[a].position.z = -this.planeDepth / 4, this.objs.lamps.data[a].position.y = this.objs.grassPlanes.data[a].position.y + this.objs.grassPlanes.data[a].size.y / 2 + this.objs.lamps.lampHeight - .2, this.objs.plafons.data[a].position.x = this.objs.lamps.data[a].position.x, this.objs.plafons.data[a].position.z = this.objs.lamps.data[a].position.z, this.objs.plafons.data[a].position.y = this.objs.lamps.data[a].position.y + 1, this.objs.bulbs.data[a].position.x = this.objs.lamps.data[a].position.x, this.objs.bulbs.data[a].position.z = this.objs.lamps.data[a].position.z, this.objs.bulbs.data[a].position.y = this.objs.lamps.data[a].position.y + 1, this.objs.grassPlanes.data[a].userData.speed = k(6, 10) / 100, this.lights.length < this.lightsCount) {
                ...
                case 2:
                    ...
                    for(let e = 0; e < this.count; e++){
                        let a = k(this.planeWidth / this.minPlaneWidthTic, this.planeWidth - 1), o = t + a / 2 + k(this.fixedDistanceHor.min, this.fixedDistanceHor.max), n = k(-1.2, 1.2) - this.planeHeight / 1.5;
                ...
                case 4:
                    ...
                    this.birdYes = !1;
                    for(let e = 0; e < this.count; e++){
                        let a = k(7 / this.minPlaneWidthTic, 18 / this.minPlaneWidthTic);
            ...
            if (this.paramsClass.gameDir == "hor") if (this.lightIntensity, this.worldClass.night && !this.worldClass.thunder) {
                ...
                this.objs.plafons.data.forEach((e, a)=>{
                    ...
                    if (e.pointLight) {
                        ...
                        const c = o ? 1 : 0;
                        this._emissive[n] = G.lerp(this._emissive[n], c, .18), this.objs.bulbs.geometryBulb.attributes.aEmissive.needsUpdate = !0;
                ...
                this.objs.plafons.data.forEach((i, e)=>{
                    ...
                    if (a) {
                        ...
                        a.position.set(m.x, m.y + 1, m.z + 2), i.glow && i.glow.position.set(m.x, m.y + 1, m.z), a.intensity = G.lerp(a.intensity, 0, .2), a.intensity <= .01 && (a.intensity = 0, this.lights.push(a), i.pointLight = null, i.userData.light = !1, i.glow && (this.scene.remove(i.glow), this.glowPool.push(i.glow), i.glow = null));
                    }
                ...
                this.objs.plafons.data.forEach((i, e)=>{
                    const a = i.pointLight;
                    !i.pointLight && this._emissive[e] === 0 || (a && (a.intensity = G.lerp(a.intensity, 0, 1), a.intensity <= .01 && (a.intensity = 0, this.lights.push(a), i.pointLight = null, i.userData.light = !1, i.glow && (this.scene.remove(i.glow), this.glowPool.push(i.glow), i.glow = null))), this.objs.plafons.plafon.setColorAt(e, this._dayColor), t = !0, this._emissive && this._emissive.length > e && (this._emissive[e] = 0));
        ...
        async showPopupInGame(s = !1, t = !1) {
            ...
            if (this.scoreClass.score > this.scoreClass.myRec && (this.scoreClass.myRec = this.scoreClass.score, i++), this.scoreClass.score > this.scoreClass.worldRec && (this.scoreClass.worldRec = this.scoreClass.score, i++), i) {
                if (this.paramsClass.gameDir === "hor") {
                    ...
                    a && (a.rec = this.scoreClass.score), await this.dataClass.submitMyScore(ysdk, `ocean${this.players.length}`, this.scoreClass.score);
                } else if (this.paramsClass.gameDir === "vert") {
                ...
                await this.dataClass.saveTableToCloud(), this.paramsClass.gameDir === "hor" ? this.scoreClass.loadRecsToHud(0, this.players.length - 1) : this.scoreClass.loadRecsToHud(1, this.players.length - 1), this.menuClass.loadRecsData();
            }
            if (this.audioClass.oceanAudio.isPlaying && this.audioClass.oceanAudio.stop(), this.audioClass.rainAudio.isPlaying && this.audioClass.rainAudio.stop(), this.gameClass.pause) document.querySelector(".popup_in_game_wrap").classList.add("popup_in_game_wrap_win"), this.hideScreen("popup_game_btn15"), this.hideScreen("popup_game_btn1"), this.levelsMode && this.showScreen("popup_game_btn4");
                ...
                if (document.querySelector(".popup_in_game_wrap").classList.add("popup_in_game_wrap_win"), this.audioClass.winAudio.isPlaying && this.audioClass.winAudio.stop(), this.audioClass.musicOn && this.audioClass.winAudio.play(), this.levelsMode < this.allLevels && this.showScreen("popup_game_btn15"), this.hideScreen("popup_game_btn4"), this.dataClass.levelCoopMode == "coop") {
                    ...
                    }), (e || a) && await this.dataClass.saveTableToCloud();
        ...
        menuInGame = ()=>{
            ...
            this.rebindButton(".popup_game_btn1", async ()=>{
                ...
                this.players.forEach((i, e, a)=>{
                    if (i.player.userData.live = !1, i.player.userData.score = 0, i.player.userData._lastMeterPos = null, i.player.userData._wasLive = !1, i.player.userData.body.setTranslation(new u(0, -5, 0)), i.player.userData.finish = !1, i.playerAliving(!0), this.levelsMode) {
    ...
    class ps {
        ...
        addInstancedDynamic(s, t, i) {
            const e = ps._toVec3(i.size), a = ps._toVec3(i.position ?? {
                ...
                y: 0,
                z: 0
            }), o = i.quaternion?.isQuaternion ? i.quaternion : new Ls, n = this.world.createRigidBody(this.RAPIER.RigidBodyDesc.dynamic().setTranslation(a.x, a.y, a.z).setRotation({
                x: o.x,
                y: o.y,
                z: o.z,
                w: o.w
    ...
    class xt {
        constructor(s, t, i, e, a, o){
            this.levelClass = s, this.isMobile = t, this.renderer = i, this.camera = e, this.paramsClass = a, this.audioClass = o, this.mouse = new u, this.raycaster = new We;
        }
        addKeyListeners() {
    ...
    class Pt {
        ...
        async loadRain() {
            ...
            this.rainGeometry.setAttribute("position", new ds(this.rainPositions, 3)), this.rainGeometry.setAttribute("color", new ds(s, 3)), this.rainStreakTex = this.makeRainStreakTexture(), this.rainMaterial = new Pe({
                color: 8947916,
                vertexColors: !0,
        ...
        async loadWaterSky() {
            ...
            }), this.water.rotation.x = -Math.PI / 2, this.water.position.x = 200, this.isMobile ? this.water.position.y = -2 : this.water.position.y = -2, this.sun = new u, this.sky = new Xe, this.sky.scale.setScalar(1e4), this.scene.add(this.sky);
            ...
            s.turbidity.value = 1, s.rayleigh.value = 3, s.mieCoefficient.value = 5e-4, s.mieDirectionalG.value = .8, this.parameters = {
                ...
                azimuth: 170,
                top: !1
            }, this.blackSky = new vs(new Hs(1e4, 1e4), new ks({
                color: 526362,
                side: Me,
                transparent: !0,
                opacity: 0
...
`;
        ...
        createLightningBolt(s, t, i) {
            const e = s + (Math.random() - .5) * 6, a = -4 + Math.random() * 3, o = i + (Math.random() - .5) * 6, n = e - s, l = a - t, h = o - i, c = Math.hypot(n, l, h) || 1, m = n / c, d = l / c, f = h / c, p = n / c, v = -(h / c), P = 0, y = p, _ = Math.abs(d) > .9 ? new u(1, 0, 0) : new u(0, 1, 0), R = new u(m, d, f), q = new u().crossVectors(R, _).normalize(), b = new u().crossVectors(R, q).normalize(), x = 2 + Math.random() * 2, T = 1.2, B = Math.random() * Math.PI * 2, L = 3 + Math.random() * 2.5, U = .8, ms = Math.random() * Math.PI * 2, C = 28, j = 4, $ = [];
            for(let S = 0; S <= C; S++){
                const z = S / C, A = 1 - z;
                let K = s + n * z, ls = t + l * z, Q = i + h * z;
                const X = Math.sin(z * Math.PI * x + B) * T * (.3 + .7 * A), ss = Math.sin(z * Math.PI * L + ms) * U * (.3 + .7 * A), es = (Math.random() - .5) * 2 * j * A, O = (Math.random() - .5) * 1.6 * j * A, W = Math.random() < .12 ? (Math.random() - .5) * 3.5 * A : 0;
                if (K += q.x * (X + es + W) + b.x * (ss + O * .7), ls += q.y * (X + es * .5) + b.y * (ss + O * .5), Q += q.z * (X + es + W) + b.z * (ss + O * .7), $.push(K, ls, Q), S > 3 && S < C - 3 && Math.random() < .22) {
                    ...
                    let Ps = K, Ms = ls, Cs = Q;
                    rs.push(Ps, Ms, Cs);
                    for(let Bs = 1; Bs <= xs; Bs++)Ps += (Math.random() - .5) * j * hs, Ms += -(.8 + Math.random() * .8) * hs, Cs += (Math.random() - .5) * j * hs, rs.push(Ps, Ms, Cs);
                    const zs = new Ds;
            ...
            for(let S = -1; S <= ys; S++){
                ...
                for(let W = 0; W < ss; W++){
                    const rs = W / (ss - 1), xs = .35 + .85 * rs, hs = Math.sin(rs * Math.PI * 2 + Q) * ls * (.2 + .8 * rs), Ps = v * A * K * xs + y * hs * .3, Ms = P * A * K * xs + hs * .05, Cs = y * A * K * xs - v * hs * .3, zs = W * 3 + 0, bs = W * 3 + 1, Bs = W * 3 + 2, ae = $[zs], ie = $[bs], oe = $[Bs];
                    z ? X.push(ae + (Math.random() - .5) * .05, ie + (Math.random() - .5) * .05, oe + (Math.random() - .5) * .05) : X.push(ae + Ps + (Math.random() - .5) * .2, ie + Ms + (Math.random() - .5) * .2, oe + Cs + (Math.random() - .5) * .2);
                }
                const es = new Ds;
        ...
        updateMetersFloat(s, t, i = "hor") {
            ...
            for (const d of t){
                ...
                if (p._lastMeterPos == null && (p._lastMeterPos = g), i !== "vert" && p._wasLive === !1 && p.live && (p._lastMeterPos = g), p.live) {
                    const v = g - p._lastMeterPos, P = v > a ? v : 0;
                    P !== 0 && (p.score += P, p._lastMeterPos = g);
                }
    ...
    class St {
        ...
        async loadLevels(s) {
            ...
            for(let l = 0; l < this.levelsStatus[s].length; l++){
                ...
                y.className = `status_chip ${h === "completed" ? "status_chip--completed" : h === "available" ? "status_chip--available" : "status_chip--locked"}`, y.setAttribute("data-i18n", `levels.status.${h}`), y.textContent = m, P.appendChild(y), p.append(v, P), p.addEventListener("click", ()=>{
        ...
        async loadLevelsContest() {
            ...
            for(let o = 0; o < this.allLevels; o++){
                ...
                d.className = "levels_block_status", l ? (d.setAttribute("data-i18n", `contest.player${l}`), d.textContent = E(`contest.player${l}`)) : d.textContent = "";
        ...
        processDataAfterLoad() {
            const s = (t)=>{
                ...
                if (i.some((a)=>a && a.name === "Мой рекорд")) return i;
                {
                    const a = t[3] && t[3].name === "Мой рекорд" ? {
                        pos: t[3].pos,
                        name: "Мой рекорд",
                        rec: t[3].rec
                    } : {
        ...
        async initYandexPlayer() {
            this.yandexPlayer.player || (this.yandexPlayer.player = await ysdk.getPlayer(), ysdk.leaderboards.getEntries("ocean1").then((s)=>console.log(s)), ysdk.leaderboards.getEntries("ocean2").then((s)=>console.log(s)));
        }
        ...
        async loadLeaderboardsTop3(s) {
            ...
            const t = this.yandexPlayer?.player?.getUniqueID ? this.yandexPlayer.player.getUniqueID() : null, i = async (e)=>{
                try {
                    ...
                    n.sort((d, f)=>f.rec - d.rec);
                    let l = [];
                    if (o && t) {
                        const d = o.rank || 0, f = typeof o.score == "number" ? o.score : 0;
                        if (n.some((v)=>v.uid === t)) l = n.slice(0, 3).map((v)=>({
                                pos: (v.uid === t, v.pos),
                                name: v.uid === t ? "Мой рекорд" : v.name,
                                rec: v.rec
                            }));
                        else {
                            const v = n.filter((y)=>y.uid !== t).slice(0, 2).map((y)=>({
                                    pos: y.pos,
                                    name: y.name,
                                    rec: y.rec
                                })), P = {
                        ...
                        const g = this.leaderboardPlacement[e];
                        if (g) {
                            const v = g.group === "hor" ? this.table.hor[g.row] : this.table.vert[g.row];
                            v && v[0] && (v[0].rec = f);
                        }
                    ...
                    if (!l.some((d)=>d.name === "Мой рекорд") && o && t) {
                        const d = o.rank || 0, f = typeof o.score == "number" ? o.score : 0;
                        c[3] = {
                            pos: d,
                            name: "Мой рекорд",
                    ...
                    console.warn(`Leaderboard ${e} smart top-3 failed:`, a);
        ...
        async submitMyScore(s, t, i) {
            const e = Number(i) || 0;
            try {
                const a = await s.isAvailableMethod("leaderboards.setScore");
                if (console.log(`isAvailableMethod ${a}`), !a) return;
    ...
    class kt {
        ...
        async loadTexture() {
            const s = new Ce, [t, i, e] = await Promise.all([
        ...
        async loadModels() {
            await new Zs().loadAsync("models/bird/bird.glb").then((i)=>{
                ...
                e.scale.x = 2, e.scale.y = 2, e.scale.z = 2, e.position.y = 0, e.rotation.y = -Math.PI / 3, this.angryBirdModel = e, this.angryBirdModel.userData.mixer = new st(this.angryBirdModel), this.angryBirdModel.userData.action = this.angryBirdModel.userData.mixer.clipAction(a[0]), this.angryBirdModel.userData.action.play(), this.angryBirdModel.userData.clock = new Gs, this.angryBirdModel.traverse((n)=>{
    ...
    let Qs, Lt = new Gs, De, ns, ws, Z, M, D, Ss, N, As, H, gs, Ys = !1, $s = !1, w = new _t;
    const us = new et;
    ...
    function Se() {
        ...
        const e = G.degToRad(4), a = G.degToRad(90);
        i = G.clamp(i, e, a), I.fov = G.radToDeg(i), I.aspect = t, I.updateProjectionMatrix(), F.setSize(r, s);
    }
    ...
    async function Et() {
        xe(!0), H = new St, gt(), gs = new kt, await gs.loadModels(), await gs.loadBoostsModel(), Y.setAttribute("style", "width:30%"), await gs.loadTexture(), await Tt(), Y.setAttribute("style", "width:30%"), D = new wt, await D.loadAudio(), Y.setAttribute("style", "width:60%"), await H.loadTableFromCloud(), await H.loadLeaderboardsTop3(ysdk), await H.loadLevels(0), await H.loadLevelsContest(), Y.setAttribute("style", "width:100%"), ns = new Ct(ke, H.loadLevels, w, D, H), xe(!1), Y.setAttribute("style", "width:0%"), ysdk.features.LoadingAPI.ready(), ysdk.features.GameplayAPI.stop();
    }
    await Et();
    async function Tt() {
        [
    ...
    async function Ht(r) {
        const s = await Le(()=>import("./@dimforge-BObwuXYQ.js").then(async (m)=>{
                await m.__tla;
                return m;
            }), [], import.meta.url);
        Qs = new s.World(new s.Vector3(0, -9.81, 0)), De = new s.EventQueue(!0), Z = new ps(Qs, s), As = new jt(I, H), ws = new Pt(us, I, F, N, Ks, D), M = new vt(us, D, Z, F, I, Ks, N, ws, ke, H, w, je, _e, As, ns, gs);
        for(let t = 0; t < r; t++)M.players.push(new bt(H, us, D, M, N, I, w, gs));
        Ss = new xt(M, Ks, F, I, N, D), Ss.addKeyListeners();
    }
    async function Ft() {
        await ws.loadWorld(), D.musicOn && D.backAudio.play(), D.musicOn && D.oceanAudio.play();
    }
    async function Rt(r) {
        await M.createLevel(r), await M.loadPlayers(), await M.loadEnvironments(), M.objs.grassPlanes.data.length > 0 && M.objs.grassPlanes.data.forEach((s, t)=>{
            M.objs.grassPlanes.data[t].userData.collide.setCollisionGroups(Fs([
                0
            ], [
                1
            ]));
        }), M.players.length > 0 && M.players.forEach((s, t)=>{
            M.players[t].player.userData.collider.setCollisionGroups(Fs([
                1
            ], [
                0,
                1
    ...
    async function ke(r, s, t = !1) {
        It(), ns.toggleLoader(!0), N = new Dt, await Ht(r), Y.setAttribute("style", "width:30%"), M.gameNum = s, await Ft(), Y.setAttribute("style", "width:60%"), await Rt(t), Y.setAttribute("style", "width:90%"), N.gameDir === "hor" ? As.loadRecsToHud(0, M.players.length - 1) : As.loadRecsToHud(1, M.players.length - 1), N.dataLoaded = !0, w.gameStarting = !0, H.gameInit = !0, Y.setAttribute("style", "width:100%"), setTimeout(()=>{
            ns.toggleLoader(!1), Y.setAttribute("style", "width:0%");
        }, 1e3);
    }
    function It() {
        I.position.y = 2, I.position.x = 0, F.toneMappingExposure = 1.05, Ss?.removedKeyListeners(), ws = null, Z = null, M = null, Ss = null, N = null, As = null;
    }
    function Gt() {
        if (w.gameStarting && document.querySelector(".menu_in_game").classList.contains("hidden_screen") && N.dataLoaded && M.showScreen("menu_in_game"), H.gameInit && w.gameStarting && !M.levelsMode && document.querySelector(".hud").classList.contains("hidden_screen") && N.dataLoaded ? (ns.showScreen("hud"), ns.hideScreen("level_hud_wrap")) : !H.gameInit && !document.querySelector(".hud").classList.contains("hidden_screen") && (ns.hideScreen("hud"), ns.showScreen("level_hud_wrap")), H.gameInit && w.gameStarting && M.levelsMode && !document.querySelector(".player_panel_rec").classList.contains("hidden_screen") ? document.querySelectorAll(".player_panel_rec").forEach((r, s, t)=>{
            r.classList.add("hidden_screen");
        }) : H.gameInit && w.gameStarting && !M.levelsMode && document.querySelector(".player_panel_rec").classList.contains("hidden_screen") && document.querySelectorAll(".player_panel_rec").forEach((r, s, t)=>{
            r.classList.remove("hidden_screen");
        }), w.gameStarting ? (je.update(Is), _e.update(Is), Ys || (ysdk.features.GameplayAPI.start(), Ys = !0, $s = !1)) : $s || (ysdk.features.GameplayAPI.stop(), $s = !0, Ys = !1), N.dataLoaded && w.gameStarting) {
            M.players.forEach((r, s, t)=>{
                r.playerMove();
            }), ws.updateLighting(), M.levelAnimate(I), M.cameraMove(I);
    ...
    document.querySelector(".pause_btn_wrap").addEventListener("click", ()=>{
        !w.pause && w.gameStarting && (w.pause = !w.pause, w.pause && (M.showPopupInGame(), w.gameStarting = !1, D.togglePauseAll(!w.gameStarting), M.showScreen("popup_game_btn_close")));
    });
    document.querySelector(".popup_game_btn_close").addEventListener("click", ()=>{
        (w.pause || w.gameStarting) && (w.pause = !w.pause, w.gameStarting = !0, D.togglePauseAll(!w.gameStarting), ws.rain && !D.rainAudio.isPlaying && D.rainAudio.play(), D.oceanAudio.isPlaying || D.oceanAudio.play(), M.hideScreen("popup_in_game"), M.hideScreen("popup_game_btn_close"));
    });
    ...
    function Nt() {
        ...
        const n = ()=>{
            ...
            const P = t.getBoundingClientRect().height, _ = Math.max(p / g * P, 24), R = g - p, q = P - _, b = R > 0 ? v / R * q : 0;
            ...
            if (!e || !s || !i || !t) return;
            const v = (p.touches ? p.touches[0].clientY : p.clientY) - a, P = t.getBoundingClientRect().height, y = s.clientHeight, _ = s.scrollHeight, R = Math.max(1, P - i.offsetHeight), q = (_ - y) / R;
            s.scrollTop = o + v * q;
        }, d = ()=>{
