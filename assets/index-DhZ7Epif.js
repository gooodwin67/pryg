import { M as v, B as w, a as E, b as H, G as B, V as u, T as V, R as P, c as U, A as Z, d as Q, P as C, e as ee, f as te, H as se, D as ie, O as ae, g as oe, h as re, i as F, W as ne, S as le, j as he, k as de, C as X, l as pe, m as _, n as ce, o as ue, p as ye, q as I, r as me, s as fe, t as ge, u as we, v as Me, w as ve, x as De, y as xe } from "./three-CiPLpyn3.js";
(async ()=>{
    (function() {
    ...
    const Le = "modulepreload", be = function(r, e) {
        return new URL(r, e).href;
    }, O = {}, Ce = function(e, t, s) {
        let i = Promise.resolve();
        if (t && t.length > 0) {
            let l = function(h) {
                return Promise.all(h.map((m)=>Promise.resolve(m).then((L)=>({
                            status: "fulfilled",
                            value: L
                        }), (L)=>({
                            status: "rejected",
            ...
            i = l(t.map((h)=>{
                if (h = be(h, s), h in O) return;
                O[h] = !0;
                const m = h.endsWith(".css"), L = m ? '[rel="stylesheet"]' : "";
                if (!!s) for(let b = o.length - 1; b >= 0; b--){
                    const S = o[b];
                    if (S.href === h && (!m || S.rel === "stylesheet")) return;
                }
                else if (document.querySelector(`link[href="${h}"]${L}`)) return;
                const f = document.createElement("link");
                if (f.rel = m ? "stylesheet" : Le, m || (f.as = "script"), f.crossOrigin = "", f.href = h, d && f.setAttribute("nonce", d), document.head.appendChild(f), m) return new Promise((b, S)=>{
    ...
    class Se {
        ...
        playerMove() {
            ...
            if (g(this.player, this.levelClass.topPlanes) ? this.player.userData.onGround = !0 : this.player.userData.onGround = !1, this.playerModel.position.x = this.player.position.x, this.playerModel.position.y = this.player.position.y - this.playerHeight / 2, this.playerModel.position.z = this.player.position.z, this.playerOut.position.copy(this.player.position), this.playerOut.rotation.copy(this.player.rotation), g(this.player, this.levelClass.boostHatMeshes) && (this.player.userData.canFly && (this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].position.copy(new u(this.player.userData.head.getWorldPosition(new u).x - .05, this.player.userData.head.getWorldPosition(new u).y + .15, this.player.userData.head.getWorldPosition(new u).z + .1)), this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].children[0].children[1].rotation.y += .4), this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].userData.fly || (this.player.userData.numHatBoost = this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes)), this.player.userData.canFly = !0, this.player.userData.hatBoost = 2), this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].userData.fly = !0), this.playerModel.position.y < -2 && (this.player.userData.live = !1), !this.player.userData.live) this.player.userData.body.setTranslation(this.player.userData.startPos), this.player.userData.readyJump = !1, this.player.userData.canFly = !1, this.player.userData.hatBoost = 0, this.player.userData.numHatBoost = 0, this.player.userData.jumping = !1, this.player.userData.playerPowerJump = 1, this.player.userData.onGround = !1, this.player.userData.body.setLinvel({
                x: 0,
                y: 0,
                z: 0
            }, !0);
            else {
                const e = this.player.userData.readyJump ? Math.PI / 2 : 0, t = this.player.userData.readyJump ? -Math.PI / 2 : 0, s = this.player.userData.body.linvel().y < -1 ? Math.PI / 1.5 : 0, i = this.player.userData.body.linvel().y < -1 ? -Math.PI / 1.5 : 0, a = this.player.userData.readyJump ? Math.PI / 8 : 0, o = this.player.userData.body.linvel().y < -.4 ? Math.PI / 7 : 0, n = this.player.userData.body.linvel().y > .4 ? Math.PI / -5.9 : 0, d = this.player.userData.readyJump ? .75 : 1.18, l = this.player.userData.readyJump ? .75 : .15;
                this.rightHand.rotation.z = this.lerp(this.rightHand.rotation.z, e, .1), this.leftHand.rotation.z = this.lerp(this.leftHand.rotation.z, t, .1), this.rightHand.rotation.z = this.lerp(this.rightHand.rotation.z, s, .1), this.leftHand.rotation.z = this.lerp(this.leftHand.rotation.z, i, .1), this.head.rotation.x = this.lerp(this.head.rotation.x, a, .1), this.head.position.y = this.lerp(this.head.position.y, d, .1), this.head.position.z = this.lerp(this.head.position.z, l, .1), this.head.rotation.z = this.lerp(this.head.rotation.z, o, .1), this.head.rotation.z = this.lerp(this.head.rotation.z, n, .1);
                const h = this.player.userData.onGround ? Math.PI : Math.PI / 1.2;
                this.playerModel.rotation.y = this.lerp(this.playerModel.rotation.y, h, .4);
    ...
    class Ae {
        constructor(e, t, s, i, a, o){
            this.scene = e, this.audioClass = t, this.physicsClass = s, this.renderer = i, this.camera = a, this.isMobile = o, this.planeWidth = 4, this.planeHeight = 10, this.geometryPlane = new w(this.planeWidth * 1.5, this.planeHeight, 1), this.materialPlane = new E({
                color: 52224
            }), this.plane = new v(this.geometryPlane, this.materialPlane), this.plane.receiveShadow = !0, this.plane.castShadow = !0, this.plane.position.y = -this.planeHeight / 2, this.plane.userData.name = "plane", this.planes = [], this.planeTop = new v(new w(this.geometryPlane.parameters.width, .4, 1.2), new H({
                color: 13421568,
                transparent: !0,
                opacity: 0
            })), this.planeTop.position.y = this.plane.position.y + this.planeHeight / 2 + .1, this.planeTop.userData.direction = 1, this.topPlanes = [], this.planeGrass = new v(new w(this.geometryPlane.parameters.width, .5, 1.2), new E({
                color: 52224,
                transparent: !0,
                opacity: 1
            })), this.planeGrass.position.y = this.plane.position.y + this.planeHeight / 2 + .1, this.planeGrass.castShadow = !0, this.planeGrass.receiveShadow = !0, this.planeGrass.userData.name = "tops", this.planeGrass.userData.direction = 1, this.grassPlanes = [], this.planeSensor = new v(new w(this.geometryPlane.parameters.width, .4, 1.2), new E({
                color: 65280,
                transparent: !0,
                opacity: 0
        ...
        async createLevel() {
            switch(await this.loadTexture(), await this.loadBoostsModel(), await this.loadEnvironmentModel(), this.cameraMove(this.camera), this.getHorizontalWorldBounds(), this.gameNum){
                ...
                case 2:
                    ...
                    let e = -2.5;
                    for(let s = 0; s < 50; s++){
                        let i = this.plane.clone(), a = this.planeTop.clone(), o = this.planeGrass.clone(), n = D(this.planeWidth / 8, this.planeWidth), d = D(2, 3), l = e + n / 2 + d, h = D(-1, 1) - this.planeHeight / 2;
                        s > 0 && (this.changeMeshWidth(i, n), this.changeMeshWidth(a, n + .3), this.changeMeshWidth(o, n + .3)), s > 0 ? (i.position.x = l, i.position.y = h + this.planeHeight / 6, a.position.x = l, a.position.y = h + this.planeHeight / 1.5 + .2, o.position.x = l, o.position.y = h + this.planeHeight / 1.5) : a.position.y = o.position.y + .2, this.planes.push(i), this.topPlanes.push(a), this.grassPlanes.push(o), e = l + n / 2;
                    }
                    break;
                case 3:
                case 4:
                    ...
                    for(let s = 0; s < 50; s++){
                        ...
                        a.userData.speed = D(4, 10) / 100;
                        let n = D(this.bounds.rightX / 2, this.bounds.rightX / 12), d = D(3, 4), l = t + d;
                        i.position.y = l, a.position.y = l, o.position.y = l, s > 0 ? (this.changeMeshWidth(i, n + .3), this.changeMeshWidth(a, n + .3), this.changeMeshWidth(o, n + .5)) : (this.changeMeshWidth(i, 10), this.changeMeshWidth(a, 10), this.changeMeshWidth(o, 10)), this.topPlanes.push(i), this.grassPlanes.push(a), this.sensorPlanes.push(o), t = l;
                    }
                    break;
            }
        }
        getHorizontalWorldBounds(e = 0) {
            ...
            if (t.unproject(this.camera), s.unproject(this.camera), this.camera.isPerspectiveCamera) {
                const i = this.camera.position, a = t.clone().sub(i).normalize(), o = s.clone().sub(i).normalize(), n = (e - i.z) / a.z, d = i.clone().add(a.multiplyScalar(n)), l = i.clone().add(o.multiplyScalar(n));
                this.bounds = {
                    leftX: d.x,
                    rightX: l.x
                };
    ...
    class Ee {
        ...
        addPhysicsToObject(e) {
            ...
            e.rotation.set(0, 0, 0), new U().setFromObject(e);
            const a = He(e);
    ...
    function He(r) {
        if (r.isMesh && r.geometry) {
            const t = r.geometry;
            return t.boundingBox || t.computeBoundingBox(), t.boundingBox.getSize(z), z.multiply(r.scale), z.clone();
        }
        return new U().setFromObject(r).getSize(new u);
    }
    class ke {
        constructor(){
    ...
    class Re {
        constructor(e, t, s, i){
            this.levelClass = e, this.isMobile = t, this.renderer = s, this.camera = i, this.camera = i, this.mouse = new u, this.raycaster = new ee, this.addKeyListeners();
        }
        addKeyListeners() {
    ...
    class _e {
        constructor(e, t, s, i){
            this.scene = e, this.camera = t, this.levelClass = s, this.renderer = i, this.ambientLight = new te(11184810, 1), this.hemiLight = new se(16777215, 16777215, 2), this.hemiLight.groundColor.setHSL(.095, 1, .75), this.hemiLight.position.set(0, 10, 0), this.dirLight = new ie(16777215, 2), this.dirLight.position.set(0, 5, 5), this.dirLight.castShadow = !0, this.dirLight.shadow.camera.far = 100, this.topLight = 1e3, this.targetObject = new ae, this.dirLight.target = this.targetObject, this.helper = new oe(this.dirLight, 3), this.water;
            const a = 50, o = 50, n = 2;
            this.pointLight = new re(16777215, n, a, o), this.pointLight.position.set(30, 40, 0), this.pointLight.lookAt(0, 0, 0);
        }
        async loadWaterSky() {
            this.waterGeometry = new F(1e3, 500), this.water = new ne(this.waterGeometry, {
                textureWidth: 500,
                textureHeight: 500,
                waterNormals: new V().load("textures/waternormals.jpg", function(l) {
                    l.wrapS = l.wrapT = P;
                }),
                sunDirection: new u,
                sunColor: 16777215,
            ...
            }), this.water.rotation.x = -Math.PI / 2, this.water.position.y = 0, this.sun = new u, this.sky = new le, this.sky.scale.setScalar(1e4), this.scene.add(this.sky);
            ...
            e.turbidity.value = 1, e.rayleigh.value = 3, e.mieCoefficient.value = 5e-4, e.mieDirectionalG.value = .8, this.parameters = {
                ...
                azimuth: 170,
                top: !1
            }, this.blackSky = new v(new F(1e4, 1e4), new he({
                color: 526362,
                side: de,
                transparent: !0,
                opacity: 0
            ...
            for(let l = 0; l < t; l++){
                ...
                const h = new X().setHSL(.5 + Math.random() * .1, .6 + Math.random() * .3, .85 + Math.random() * .15);
...
`;
            this.materialStars = new ce({
                ...
                depthWrite: !1,
                blending: ue
            }), this.stars = new ye(o, this.materialStars), this.stars.layers.set(1), this.scene.add(this.stars), this.camera.layers.enable(1);
        }
        updateSky() {
            this.stars.position.x = this.camera.position.x;
            const e = I.degToRad(90 - this.parameters.elevation), t = I.degToRad(this.parameters.azimuth);
            if (this.sun.setFromSphericalCoords(1, e, t), this.sky.material.uniforms.sunPosition.value.copy(this.sun), this.water.material.uniforms.sunDirection.value.copy(this.sun).normalize(), this.levelClass.gameDir == "hor" && (this.sun.y < -.07 && this.materialStars.uniforms.opacity.value < 1 ? (this.materialStars.uniforms.opacity.value += .001, this.blackSky.material.opacity < .8 && (this.blackSky.material.opacity += .001)) : this.sun.y > -.07 && this.materialStars.uniforms.opacity.value > 0 && (this.materialStars.uniforms.opacity.value -= .001, this.blackSky.material.opacity -= .001), this.parameters.elevation < -8 ? this.parameters.top = !0 : this.parameters.elevation > 6 && (this.parameters.top = !1), this.parameters.top ? (this.parameters.elevation += .003, this.dirLight.intensity += 3e-4, this.dirLight.intensity = Math.max(.5, Math.min(2, this.dirLight.intensity)), this.hemiLight.intensity += 3e-4, this.hemiLight.intensity = Math.max(.5, Math.min(2, this.hemiLight.intensity)), this.renderer.toneMappingExposure += 24e-5, this.renderer.toneMappingExposure = Math.max(.2, Math.min(1.05, this.renderer.toneMappingExposure))) : (this.parameters.elevation -= .003, this.dirLight.intensity -= 3e-4, this.dirLight.intensity = Math.max(.5, Math.min(2, this.dirLight.intensity)), this.hemiLight.intensity -= 3e-4, this.hemiLight.intensity = Math.max(.5, Math.min(2, this.hemiLight.intensity)), this.renderer.toneMappingExposure -= 3e-4, this.renderer.toneMappingExposure = Math.max(.2, Math.min(1.05, this.renderer.toneMappingExposure)))), this.levelClass.gameDir == "vert") {
    ...
    console.clear();
    let W, Y = !1, A = 0, K = 1 / 60, Be = new xe, j, T, G, M, p, k;
    const x = new me;
    x.background = new X(13230580);
    const y = new fe(25, window.innerWidth / window.innerHeight, .1, 2e3);
    y.position.z = 7;
    y.position.y = 2;
    const N = Pe();
    let $ = new ge;
    ...
    c.shadowMap.type = Me;
    c.outputColorSpace = ve;
    c.toneMapping = De;
    c.toneMappingExposure = 1.05;
    window.addEventListener("resize", We, !1);
    function We() {
        y.aspect = window.innerWidth / window.innerHeight, y.updateProjectionMatrix(), c.setSize(window.innerWidth, window.innerHeight);
    }
    async function Te(r) {
        ...
        W = new e.World(new e.Vector3(0, -9.81, 0)), j = new e.EventQueue(!0), M = new Ee(W, e), k = new ke, p = new Ae(x, k, M, c, y, N), G = new _e(x, y, p, c);
        for(let t = 0; t < r; t++)p.players.push(new Se(x, k, p));
        new Re(p, N, c, y);
    }
    async function Ge() {
    ...
    async function Ie(r, e) {
        T.toggleLoader(!0), await Te(r), p.gameNum = e, await Ge(), await Fe(), T.toggleLoader(!1), Y = !0;
    }
    T = new ze(Ie);
    function Oe() {
        if (Y) {
            p.players.forEach((r, e, t)=>{
                r.playerMove();
            }), G.updateLighting(), p.levelAnimate(y), p.cameraMove(y), $.update();
            ...
            W.step(j), c.render(x, y);
        }
    }
    c.setAnimationLoop(()=>{
        A += Be.getDelta(), A > K && (Oe(), c.render(x, y), A = A % K);
    });
})();
