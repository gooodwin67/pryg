import { B as Ps, a as ds, P as me, N as xe, b as Us, c as Hs, C as Js, M as Ms, d as fs, V as p, e as bs, Q as As, f as Pe, g as as, h as Cs, i as hs, G as Vs, E as X, j as is, D as ye, S as Ce, k as je, l as se, I as es, m as ts, n as Me, o as Ts, O as Ks, T as be, R as js, p as Bs, A as _e, q as Ae, r as Se, s as Es, t as I, u as Le, v as ke, w as B, x as ze, y as Be, H as Ee, z as He, F as Te, L as Fe, W as Re, J as Ie, K as Ne, U as ee, X as te, Y as Ge, Z as qe, _ as ae, $ as ie, a0 as We, a1 as Oe, a2 as Ue, a3 as Ve, a4 as Ye, a5 as Je, a6 as Ke } from "./three-DzE3nDLF.js";
(async ()=>{
    (function() {
    ...
    const Xe = "modulepreload", $e = function(h, s) {
        ...
        if (t && t.length > 0) {
            let r = function(d) {
                return Promise.all(d.map((y)=>Promise.resolve(y).then((u)=>({
                            status: "fulfilled",
                            value: u
                        }), (u)=>({
                            status: "rejected",
            ...
            e = r(t.map((d)=>{
                ...
                oe[d] = !0;
                const y = d.endsWith(".css"), u = y ? '[rel="stylesheet"]' : "";
                if (!!i) for(let b = o.length - 1; b >= 0; b--){
                    const g = o[b];
                    if (g.href === d && (!y || g.rel === "stylesheet")) return;
                }
                else if (document.querySelector(`link[href="${d}"]${u}`)) return;
                const c = document.createElement("link");
                if (c.rel = y ? "stylesheet" : Xe, y || (c.as = "script"), c.crossOrigin = "", c.href = d, l && c.setAttribute("nonce", l), document.head.appendChild(c), y) return new Promise((b, g)=>{
                    c.addEventListener("load", b), c.addEventListener("error", ()=>g(new Error(`Unable to preload CSS for ${d}`)));
                });
            }));
        }
        function a(o) {
    ...
    const et = new bs(new p(-.5, -.5, -.5), new p(.5, .5, .5)), he = new Pe, de = new As;
    function ce(h) {
        if (h?.isObject3D) {
        ...
        const s = h.position ?? h.pos ?? new p, t = st(h.size ?? 1), i = h.quaternion?.isQuaternion ? h.quaternion : h.rotation?.isEuler ? de.setFromEuler(h.rotation) : de.set(0, 0, 0, 1);
    ...
    function tt({ scene: h, maxParticles: s = 800, gravity: t = -7.8, drag: i = 2, texture: e = null, pointSize: a = .66, blending: o = xe } = {}) {
        if (!h) throw new Error("createSplashSystem: scene is required");
        function n() {
            const P = document.createElement("canvas");
            P.width = P.height = 64;
            const E = P.getContext("2d"), L = E.createRadialGradient(64 / 2, 64 / 2, 0, 64 / 2, 64 / 2, 64 / 2);
            L.addColorStop(0, "rgba(255,255,255,1)"), L.addColorStop(1, "rgba(255,255,255,0)"), E.fillStyle = L, E.fillRect(0, 0, 64, 64);
            const R = new Js(P);
            return R.anisotropy = 1, R.needsUpdate = !0, R;
        }
        const l = e || n(), r = new Float32Array(s * 3), d = new Float32Array(s * 3), y = new Float32Array(s), u = new Float32Array(s), f = new Float32Array(s), c = new Uint8Array(s), b = new Ps;
        b.setAttribute("position", new ds(r, 3)), b.setAttribute("aSize", new ds(f, 1));
        const g = new me({
            map: l,
            size: a,
            transparent: !0,
        ...
        m.userData.persistent = !0, m.frustumCulled = !1, h.add(m);
        let z = 0;
        function N() {
            for(let w = 0; w < s; w++){
                const P = (z + w) % s;
                if (!c[P]) return z = (P + 1) % s, P;
            }
            return -1;
        }
        function G(w, P, E, L, R) {
            const O = P * 3;
            w[O] = E, w[O + 1] = L, w[O + 2] = R;
        }
        return {
            trigger (w, P = 1, E = {}) {
                const { count: L = 42, spread: R = .35, up: O = 3, horiz: us = 2.2, ttl: C = [
                    .35,
                    .8
                ], sizeJitter: M = .5 } = E, Y = Math.max(1, Math.floor(L * P));
                for(let ms = 0; ms < Y; ms++){
                    const _ = N();
                    if (_ === -1) break;
                    const k = Math.sqrt(Math.random()) * R, S = Math.random() * Math.PI * 2, J = k * Math.cos(S), ns = k * Math.sin(S), Z = Math.sqrt(Math.random()), K = Math.cos(S) * us * Z * (.6 + .4 * Math.random()), Q = Math.sin(S) * us * Z * (.6 + .4 * Math.random()), ss = O * (.6 + .4 * Math.random()), U = C[0] + Math.random() * (C[1] - C[0]), W = (1 - M / 2 + Math.random() * M) * 1;
                    G(r, _, w.x + J, w.y, w.z + ns), G(d, _, K * P, ss * P, Q * P), y[_] = U, u[_] = 0, f[_] = W, c[_] = 1;
                }
                b.attributes.position.needsUpdate = !0, b.attributes.aSize.needsUpdate = !0;
            },
            update (w) {
                if (w <= 0) return;
                const P = t, E = Math.max(0, i);
                let L = !1;
                for(let C = 0; C < s; C++){
                    if (!c[C]) continue;
                    if (L = !0, u[C] += w, u[C] >= y[C]) {
                        c[C] = 0;
                        const S = C * 3;
                    ...
                    const M = C * 3;
                    d[M + 1] += P * w;
                    const Y = d[M], ms = d[M + 1], _ = d[M + 2], k = Math.max(0, 1 - E * w);
                    d[M] = Y * k, d[M + 1] = ms * k, d[M + 2] = _ * k, r[M] += d[M] * w, r[M + 1] += d[M + 1] * w, r[M + 2] += d[M + 2] * w;
                }
                L && (b.attributes.position.needsUpdate = !0);
                let R = 0, O = 0;
                for(let C = 0; C < s; C++)c[C] && (R++, O += 1 - u[C] / y[C]);
                const us = R ? .25 + .75 * (O / R) : 1;
                g.size = a * us;
            },
            get object3D () {
                return m;
            ...
            dispose () {
                h.remove(m), b.dispose(), g.dispose(), e || l.dispose();
            }
        };
    }
    function at({ scene: h, size: s = 1.5, ttl: t = .9 } = {}) {
        const i = new Hs(1, 1), e = (()=>{
            ...
            return c.clearRect(0, 0, 64, 64), c.strokeStyle = "rgba(255,255,255,0.9)", c.lineWidth = 3, c.beginPath(), c.arc(32, 32, 20, 0, Math.PI * 2), c.stroke(), new Js(f);
        })(), a = new Ms({
            map: e,
            transparent: !0,
            depthWrite: !1
        ...
        function y(f, c) {
            ...
            const b = n / t, g = s * (1 + 1.6 * b);
            o.scale.setScalar(g), a.opacity = 1 - b;
        }
        function u() {
    ...
    class it {
        ...
        async loadPlayerModel() {
            await new Vs().loadAsync("models/players/player1.glb").then((i)=>{
                const e = i.scene;
        ...
        playerMove() {
            ...
            if (this.player.userData.canFlyJumps) {
                ...
                const i = new p().setFromMatrixPosition(this.player.userData.head.matrixWorld), e = new As;
                this.player.userData.head.getWorldQuaternion(e);
                const a = new As().setFromEuler(new X(0, Math.PI / 2, 0)), o = e.clone().multiply(a), l = new p(.01, .14, .05).clone().applyQuaternion(o);
    ...
    class ot {
        constructor(s, t, i, e, a, o, n, l, r, d, y, u, f, c, b){
            this.scene = s, this.audioClass = t, this.physicsClass = i, this.renderer = e, this.camera = a, this.isMobile = o, this.paramsClass = n, this.worldClass = l, this.initMatch = r, this.gameClass = y, this.splash = u, this.ring = f, this.dataClass = d, this.scoreClass = c, this.menuClass = b, this.cameraSpeed = .01, this.levelsMode = !1, this.levelsNoFric = !1, this.allLevels = this.dataClass.allLevels, this.randomNoFric = .3, this.randomAnimateHor = .2, this.randomAnimateVert = .2, this.canShowAds = !0, this.boostHatModel, this.boostHatPropeller, this.boostHatMesh, this.boostHatModels = [], this.boostHatMeshes = [], this.boostHatCoords = [], this.angryBird, this.birdFlyingMark = 10, this.distanceToBird = 20, this.angryBirdModel, this.maxHeight = 0, this.birdYes = !0, this.canHorDie = !1, this.planeWidth = 4, this.planeHeight = 10, this.planeDepth = 1, this.minPlaneWidthTic = 1, this.fixedDistanceHor = {
                min: 1,
                max: 4
            }, this.fixedDistanceVert = {
                min: 3,
                max: 4
            }, this.count = 120, this._dayColor = new is(16777215), this._nightColor = new is(16771488), this.mksWidth = 100, this.mksHeight = 100, this.geometryPlane = new Hs(this.mksWidth, this.mksHeight), this.materialPlane = new Ms({
                color: 0,
                side: ye
            }), this.mks = new fs(this.geometryPlane, this.materialPlane), this.mks.position.z = -650, this.mks.position.y = 190, this.mks.layers.set(1);
            const g = new Ce, m = .01;
            g.moveTo(5 * m, 5 * m), g.bezierCurveTo(5 * m, 5 * m, 4 * m, 2 * m, 0 * m, 2 * m), g.bezierCurveTo(-6 * m, 2 * m, -6 * m, 7 * m, -6 * m, 7 * m), g.bezierCurveTo(-6 * m, 10 * m, -3 * m, 14 * m, 5 * m, 18 * m), g.bezierCurveTo(12 * m, 14 * m, 16 * m, 10 * m, 16 * m, 7 * m), g.bezierCurveTo(16 * m, 7 * m, 16 * m, 2 * m, 10 * m, 2 * m), g.bezierCurveTo(7 * m, 2 * m, 5 * m, 5 * m, 5 * m, 5 * m);
            const z = {
                depth: .22,
                bevelEnabled: !1,
                curveSegments: 12,
    ...
    `);
            ...
            const N = new Float32Array(this.count);
            for(let v = 0; v < this.count; v++)N[v] = 0;
            this.objs.bulbs.geometryBulb.setAttribute("aEmissive", new Me(N, 1)), this.objs.bulbs.materialBulb.onBeforeCompile = (v)=>{
                v.vertexShader = `
  ...
  ` + v.fragmentShader.replace("#include <lights_fragment_begin>", `
    ...
    `);
            ...
            function G(v = 64) {
                ...
                w.width = w.height = v;
                const P = w.getContext("2d"), E = P.createRadialGradient(v / 2, v / 2, 0, v / 2, v / 2, v / 2);
                E.addColorStop(0, "rgba(255, 235, 175, 1)"), E.addColorStop(1, "rgba(255, 235, 175, 0)"), P.fillStyle = E, P.fillRect(0, 0, v, v);
                const L = new Js(w);
                return L.anisotropy = 1, L.generateMipmaps = !1, L.needsUpdate = !0, L;
            }
            this._glowTex = G(), this._emissive = N, this.glowPool = [], this.lightsCount = 5, this.lights = [], this.lightIntensity = 25, this.bulbEmissiveIntensity = .9, this.playerOuts = [], this.players = [], this.backModel, this.backModels = [], this.leftEdge = new p(-1, 0, 0), this.rightEdge = new p(1, 0, 0), this.leftEdge.unproject(a), this.rightEdge.unproject(a), this.bounds, this.gameNum = 1, this.cam = {
        ...
        apply(s, t, i) {
            ...
            if (!e) {
                ...
                const r = new p;
                l.boundingBox.getSize(r), e = i.userData.invBaseSize = new p(1 / (r.x || 1), 1 / (r.y || 1), 1 / (r.z || 1));
            }
            this._dummy ||= new Ks;
        ...
        async loadTexture() {
            ...
            s.load("textures/plane.jpg", (t)=>{
                ...
                t.wrapS = js, t.wrapT = js, t.repeat.set(this.planeWidth / 4, this.planeHeight / 4), this.objs.planes.plane.material = i;
            }, void 0, function(t) {
                ...
                t.wrapS = js, t.wrapT = js, t.repeat.set(this.planeWidth / 1, this.planeHeight / 8), this.objs.grassPlanes.planeGrass.material = i;
            }, void 0, function(t) {
                console.error("An error happened.");
            }), s.load("textures/mks.png", (t)=>{
                const i = new Ms({
                    map: t,
                    transparent: !0,
                    opacity: 0
        ...
        async loadBarriers() {
            let s = new as(.5, .7, 1), t = new Ms({
                color: 52224,
                transparent: !0,
                opacity: 0
            });
            this.angryBird = new fs(s, t), this.angryBird.userData.name = "bird", this.angryBird.userData.speed = A(8, 13) / 100, this.angryBird.userData.flying = !1, this.angryBird.position.y = -5, this.physicsClass.addPhysicsToObject(this.angryBird);
        }
        async createLevel(s) {
            if (this.levelsMode = s, this.maxHeight = 0, this.birdFlyingMark = 10, document.querySelector(".lev_hud span").textContent = s, await this.loadTexture(), await this.loadBarriers(), await this.loadBoostsModel(), await this.loadBirdModel(), document.querySelectorAll(".player_panel")[0].classList.add("hidden_screen"), document.querySelectorAll(".player_panel")[1].classList.add("hidden_screen"), document.querySelectorAll(".player_panel")[2].classList.add("hidden_screen"), this.players.forEach((t, i, e)=>{
                ...
                if (this.paramsClass.gameDir == "hor") {
                    for(let a = 0; a < this.count; a++){
                        let o = A(this.planeWidth, this.planeWidth - 1), n = t + o / 2 + A(this.fixedDistanceHor.min, this.fixedDistanceHor.max), l = A(-1.2, 1.2) - this.planeHeight / 1.5;
                ...
                case 2:
                    ...
                    for(let e = 0; e < this.count; e++){
                        let a = A(this.planeWidth / this.minPlaneWidthTic, this.planeWidth - 1), o = t + a / 2 + A(this.fixedDistanceHor.min, this.fixedDistanceHor.max), n = A(-1.2, 1.2) - this.planeHeight / 1.5;
        ...
        getHorizontalWorldBounds(s = 0) {
            ...
            if (t.unproject(this.camera), i.unproject(this.camera), e.unproject(this.camera), a.unproject(this.camera), this.camera.isPerspectiveCamera) {
                const o = this.camera.position, n = t.clone().sub(o).normalize(), l = i.clone().sub(o).normalize(), r = e.clone().sub(o).normalize(), d = a.clone().sub(o).normalize(), y = (s - o.z) / n.z, u = (s - o.z) / d.z, f = o.clone().add(n.multiplyScalar(y)), c = o.clone().add(l.multiplyScalar(y)), b = o.clone().add(r.multiplyScalar(u)), g = o.clone().add(d.multiplyScalar(u));
                this.bounds = {
                    leftX: f.x,
                    rightX: c.x,
                    topY: b.y,
                ...
                for(let t = 0; t < this.objs.grassPlanes.data.length; t++){
                    ...
                    if (e && (a || o)) {
                        if (a) {
                            ...
                            const y = i.userData.direction * d, u = n.x + y;
                            ...
                            const y = i.userData.direction * d, u = n.y + y;
                            e.setNextKinematicTranslation({
                                x: n.x,
                                y: u,
                                z: n.z
        ...
        async loadBirdModel() {
            await new Vs().loadAsync("models/bird/bird.glb").then((i)=>{
                const e = i.scene, a = i.animations;
                e.scale.x = 2, e.scale.y = 2, e.scale.z = 2, e.position.y = 0, e.rotation.y = -Math.PI / 3, this.angryBirdModel = e, this.angryBirdModel.userData.mixer = new _e(this.angryBirdModel), this.angryBirdModel.userData.action = this.angryBirdModel.userData.mixer.clipAction(a[0]), this.angryBirdModel.userData.action.play(), this.angryBirdModel.userData.clock = new Ts;
        ...
        async loadBoostsModel() {
            await new Vs().loadAsync("models/boosts/hat.glb").then((i)=>{
                const e = i.scene;
            ...
            if (this.paramsClass.gameDir == "hor") if (this.lightIntensity, this.worldClass.night && !this.worldClass.thunder) {
                ...
                this.objs.plafons.data.forEach((a, o)=>{
                    ...
                    if (a.pointLight) {
                        ...
                        r.intensity = I.lerp(r.intensity, d, .15);
                        const y = n ? 1 : 0;
                        this._emissive[l] = I.lerp(this._emissive[l], y, .18), this.objs.bulbs.geometryBulb.attributes.aEmissive.needsUpdate = !0;
                        const u = .5 + this._emissive[l] * .8;
                        a.glow && a.glow.scale.setScalar(u);
                        const f = 1.1, c = this._emissive[l], b = 1 + f * c, g = this.objs.bulbs.baseSize, m = this.objs.bulbs.data[l];
                        m.userData._lastBulbFactor !== b && (m.size.set(g.x * b, g.y * b, g.z * b), this.apply(l, this.objs.bulbs.data, this.objs.bulbs.bulb), m.userData._lastBulbFactor = b, s = !0), !n && r.intensity <= .01 && this._emissive[l] <= .02 && (this.lights.push(r), a.pointLight = null, a.glow && (this.glowPool.push(a.glow), this.scene.remove(a.glow), a.glow = null));
                    }
                ...
                this.objs.plafons.data.forEach((e, a)=>{
                    ...
                    o && (o.intensity = I.lerp(o.intensity, 0, .2), o.intensity <= .01 && (o.intensity = 0, this.lights.push(o), e.pointLight = null, e.userData.light = !1, e.glow && (this.scene.remove(e.glow), this.glowPool.push(e.glow), e.glow = null))), this.objs.plafons.plafon.setColorAt(a, this._dayColor), i = !0, this._emissive && this._emissive.length > a && (this._emissive[a] = 0);
                    const n = 1.1, l = this._emissive[a], r = 1 + n * l, d = this.objs.bulbs.baseSize, y = this.objs.bulbs.data[a];
                    y.userData._lastBulbFactor !== r && (y.size.set(d.x * r, d.y * r, d.z * r), this.apply(a, this.objs.bulbs.data, this.objs.bulbs.bulb), y.userData._lastBulbFactor = r, s = !0);
                ...
                this.objs.plafons.data.forEach((e, a)=>{
                    ...
                    if (e.pointLight) {
                        ...
                        l.intensity = I.lerp(l.intensity, r, .15);
                        const d = o ? 1 : 0;
                        this._emissive[n] = I.lerp(this._emissive[n], d, .18), this.objs.bulbs.geometryBulb.attributes.aEmissive.needsUpdate = !0;
                        const y = .8 + this._emissive[n] * .8;
                        e.glow && e.glow.scale.setScalar(y);
                        const u = 1, f = this._emissive[n], c = 1 + u * f, b = this.objs.bulbs.baseSize, g = this.objs.bulbs.data[n];
                        g.userData._lastBulbFactor !== c && (g.size.set(b.x * c, b.y * c, b.z * c), this.apply(n, this.objs.bulbs.data, this.objs.bulbs.bulb), g.userData._lastBulbFactor = c, s = !0), !o && l.intensity <= .01 && this._emissive[n] <= .02 && (this.lights.push(l), e.pointLight = null, e.glow && (this.glowPool.push(e.glow), this.scene.remove(e.glow), e.glow = null));
                    }
                }), s && (this.objs.bulbs.bulb.instanceMatrix.needsUpdate = !0);
            } else {
                let t = !1;
        ...
        cameraMove(s, t = this.dt.getDelta()) {
            switch(this.gameNum){
                ...
                case 3:
                    ...
                    break;
                case 4:
                    this.gameClass.gameStarting && this.players[this.maxSpeed()].player.userData.body.linvel().y > -20 && (s.position.y = this.players[this.maxSpeed()].player.position.y + 3.5), s.position.x = 0, s.position.z = this.isMobile ? 20 : 32, s.lookAt(s.position.x, s.position.y - 2, 0), this.mks.material.opacity = this.worldClass.blackSky.material.opacity, s.position.y > 40 && (this.mks.position.x -= .01, this.mks.position.y += .01, this.mks.position.z -= .01);
                    break;
            }
        }
        damp(s, t, i, e) {
        ...
        showPopupInGame(s = !1, t = !1) {
            ...
            let i = 0;
            if (this.scoreClass.score > this.scoreClass.myRec && (this.scoreClass.myRec = this.scoreClass.score, i++), this.scoreClass.score > this.scoreClass.worldRec && (this.scoreClass.worldRec = this.scoreClass.score, i++), i && (this.paramsClass.gameDir === "hor" ? this.dataClass.table.hor[this.players.length - 1].find((e, a, o)=>e.name === "Мой рекорд").rec = this.scoreClass.score : this.paramsClass.gameDir === "vert" && (this.dataClass.table.vert[this.players.length - 1].find((e, a, o)=>e.name === "Мой рекорд").rec = this.scoreClass.score), this.dataClass.saveLocalData(), this.dataClass.loadLocalData(), this.paramsClass.gameDir === "hor" ? this.scoreClass.loadRecsToHud(0, this.players.length - 1) : this.scoreClass.loadRecsToHud(1, this.players.length - 1), this.menuClass.loadRecsData()), this.audioClass.oceanAudio.isPlaying && this.audioClass.oceanAudio.stop(), this.audioClass.rainAudio.isPlaying && this.audioClass.rainAudio.stop(), this.gameClass.pause) document.querySelector(".popup_in_game_wrap").classList.add("popup_in_game_wrap_win"), this.hideScreen("popup_game_btn15"), this.hideScreen("popup_game_btn1"), this.levelsMode && this.showScreen("popup_game_btn4");
                ...
                if (document.querySelector(".popup_in_game_wrap").classList.add("popup_in_game_wrap_win"), this.audioClass.winAudio.isPlaying && this.audioClass.winAudio.stop(), this.audioClass.musicOn && this.audioClass.winAudio.play(), this.levelsMode < this.allLevels && this.showScreen("popup_game_btn15"), this.hideScreen("popup_game_btn4"), this.dataClass.levelCoopMode == "coop") {
                    ...
                    this.players.forEach((o, n, l)=>{
                        this.levelsMode == this.allLevels && (this.dataClass.table.player.bonusHeart[n] = 2, e = !0), this.levelsMode + 1 > this.dataClass.table.player.levels[n] && (this.dataClass.table.player.levels[n] = this.levelsMode, a = !0);
                    }), (e || a) && this.dataClass.saveLocalData();
                } else this.dataClass.levelCoopMode == "contest" && this.players.forEach((e, a, o)=>{
                    e.player.userData.finish && this.dataClass.table.levelsStatusContest[this.levelsMode - 1] != a + 1 && (console.log(123), this.dataClass.table.levelsStatusContest[this.levelsMode - 1] = a + 1, this.dataClass.saveLocalData());
                });
        ...
        menuInGame = ()=>{
            this.rebindButton(".popup_game_btn1", ()=>{
                ...
                this.audioClass.hardStopAll(), this.players.forEach((t, i, e)=>{
                    if (t.player.userData.live = !1, t.player.userData.score = 0, t.player.userData._lastMeterPos = null, t.player.userData._wasLive = !1, t.player.userData.body.setTranslation(new p(0, -5, 0)), t.player.userData.finish = !1, t.playerAliving(!0), this.levelsMode) {
                ...
                this.audioClass.hardStopAll(), this.paramsClass.dataLoaded = !1, Is(this.scene), this.audioClass.stopMusic(0), setTimeout(()=>{
                ...
                this.audioClass.hardStopAll(), this.gameClass.pause = !1, this.gameClass.showGamePopup = !1, this.hideScreen("popup_in_game"), this.showScreen("main_screen"), this.paramsClass.dataLoaded = !1, Is(this.scene), this.audioClass.stopMusic(0), this.dataClass.gameInit = !1;
            }), this.rebindButton(".popup_game_btn4", ()=>{
                this.audioClass.hardStopAll(), this.gameClass.pause = !1, this.gameClass.showGamePopup = !1, this.hideScreen("popup_in_game"), this.dataClass.levelCoopMode == "contest" ? this.showScreen("levels_game_screen_contest") : this.showScreen("levels_game_screen"), this.paramsClass.dataLoaded = !1, Is(this.scene), this.audioClass.stopMusic(0), this.dataClass.gameInit = !1;
            });
        };
        hideScreen(s) {
    ...
    class cs {
        ...
        addInstancedDynamic(s, t, i) {
            const e = cs._toVec3(i.size), a = cs._toVec3(i.position ?? {
                ...
                y: 0,
                z: 0
            }), o = i.quaternion?.isQuaternion ? i.quaternion : new As, n = this.world.createRigidBody(this.RAPIER.RigidBodyDesc.dynamic().setTranslation(a.x, a.y, a.z).setRotation({
                x: o.x,
                y: o.y,
                z: o.z,
                w: o.w
    ...
    class nt {
        ...
        async loadAudio() {
            ...
            await s.loadAsync("audio/back1.mp3").then((t)=>{
                ...
                this.winAudio = new B(this.listener), this.winAudio.setBuffer(t), this.winAudio.setLoop(!1), this.winAudio.setRefDistance(200), this.winAudio.setVolume(2), this.musics.push({
                    name: "win",
                    music: this.winAudio
                });
            }).catch((t)=>{
    ...
    class lt {
        constructor(s, t, i, e, a, o){
            this.levelClass = s, this.isMobile = t, this.renderer = i, this.camera = e, this.paramsClass = a, this.audioClass = o, this.mouse = new p, this.raycaster = new ze;
        }
        addKeyListeners() {
    ...
    class rt {
        constructor(s, t, i, e, a, o){
            this.scene = s, this.camera = t, this.renderer = i, this.paramsClass = e, this.isMobile = a, this.audioClass = o, this.ambientLight = new Be(11184810, 1), this.hemiLight = new Ee(16777215, 16777215, 2), this.hemiLight.groundColor.setHSL(.095, 1, .75), this.hemiLight.position.set(0, 10, 0), this.dirLight = new He(16777215, 2), this.dirLight.position.set(0, 5, 5), this.dirLight.castShadow = !0, this.dirLight.shadow.camera.far = 100, this.topLight = 1e3, this.targetObject = new Ks, this.dirLight.target = this.targetObject, this.helper = new Te(this.dirLight, 3), this.water, this.night = !1, this._prevCamX = this.camera.position.x, this.thunder = !1, this.thunderStart = !1, this.isThunderActive = !1, this.thunderEndTimestampMs = 0, this.nextThunderFlashTimestampMs = 0, this.minThunderIntervalMs = 1e3, this.maxThunderIntervalMs = 3e3, this.currentThunderIndex = 0, this.rain = !1, this.rainStart = !1, this.isRainActive = !1, this.rainEndTimestampMs = 0, this.activeLightningLines = [], this.lightningMaterialBase = new Fe({
                color: 16777215,
                transparent: !0,
                opacity: 1,
                blending: Es,
                depthWrite: !1
            }), this.clock = new Ts, this.deltaSeconds, this.lightningFade = 0, this.rainDropCount = 1500, this.rainAreaHalfWidth = 10, this.rainAreaHalfDepth = 22, this.rainTopY = 7, this.rainBottomY = -2, this.rainGeometry = new Ps, this.rainPositions = new Float32Array(this.rainDropCount * 3), this.rainVelocities = new Float32Array(this.rainDropCount), this.rainWindPhase = new Float32Array(this.rainDropCount);
        }
        async loadRain() {
            ...
            this.rainGeometry.setAttribute("position", new ds(this.rainPositions, 3)), this.rainGeometry.setAttribute("color", new ds(s, 3)), this.rainStreakTex = this.makeRainStreakTexture(), this.rainMaterial = new me({
                ...
                depthWrite: !0,
                blending: Es
            }), this.rainPoints = new Us(this.rainGeometry, this.rainMaterial), this.rainPoints.layers.set(1);
        }
        async loadWaterSky() {
            this.waterGeometry = new Hs(900, 500), this.water = new Re(this.waterGeometry, {
                textureWidth: 500,
                textureHeight: 500,
                waterNormals: new be().load("textures/waternormals.jpg", function(r) {
                    r.wrapS = r.wrapT = js;
                }),
                sunDirection: new p,
                sunColor: 16755370,
            ...
            }), this.water.rotation.x = -Math.PI / 2, this.water.position.x = 200, this.isMobile ? this.water.position.y = -2 : this.water.position.y = -2, this.sun = new p, this.sky = new Ie, this.sky.scale.setScalar(1e4), this.scene.add(this.sky);
            ...
            s.turbidity.value = 1, s.rayleigh.value = 3, s.mieCoefficient.value = 5e-4, s.mieDirectionalG.value = .8, this.parameters = {
                ...
                azimuth: 170,
                top: !1
            }, this.blackSky = new fs(new Hs(1e4, 1e4), new Ms({
                color: 526362,
                side: ye,
                transparent: !0,
                opacity: 0
...
`;
            this.materialStars = new Ne({
                ...
                depthWrite: !1,
                blending: Es
            }), this.stars = new Us(o, this.materialStars), this.stars.layers.set(1), this.scene.add(this.stars), this.camera.layers.enable(1);
        }
        updateSky() {
        ...
        createLightningBolt(s, t, i) {
            const e = s + (Math.random() - .5) * 6, a = -4 + Math.random() * 3, o = i + (Math.random() - .5) * 6, n = e - s, l = a - t, r = o - i, d = Math.hypot(n, l, r) || 1, y = n / d, u = l / d, f = r / d, c = n / d, g = -(r / d), m = 0, z = c, N = Math.abs(u) > .9 ? new p(1, 0, 0) : new p(0, 1, 0), G = new p(y, u, f), v = new p().crossVectors(G, N).normalize(), w = new p().crossVectors(G, v).normalize(), P = 2 + Math.random() * 2, E = 1.2, L = Math.random() * Math.PI * 2, R = 3 + Math.random() * 2.5, O = .8, us = Math.random() * Math.PI * 2, C = 28, M = 4, Y = [];
            for(let _ = 0; _ <= C; _++){
                const k = _ / C, S = 1 - k;
                let J = s + n * k, ns = t + l * k, Z = i + r * k;
                const K = Math.sin(k * Math.PI * P + L) * E * (.3 + .7 * S), Q = Math.sin(k * Math.PI * R + us) * O * (.3 + .7 * S), ss = (Math.random() - .5) * 2 * M * S, U = (Math.random() - .5) * 1.6 * M * S, W = Math.random() < .12 ? (Math.random() - .5) * 3.5 * S : 0;
                if (J += v.x * (K + ss + W) + w.x * (Q + U * .7), ns += v.y * (K + ss * .5) + w.y * (Q + U * .5), Z += v.z * (K + ss + W) + w.z * (Q + U * .7), Y.push(J, ns, Z), _ > 3 && _ < C - 3 && Math.random() < .22) {
                    const ls = [], vs = 3 + Math.floor(Math.random() * 2), rs = .25 + Math.random() * .35;
                    let ws = J, Ds = ns, xs = Z;
                    ls.push(ws, Ds, xs);
                    for(let zs = 1; zs <= vs; zs++)ws += (Math.random() - .5) * M * rs, Ds += -(.8 + Math.random() * .8) * rs, xs += (Math.random() - .5) * M * rs, ls.push(ws, Ds, xs);
                    const ks = new Ps;
                    ks.setAttribute("position", new ee(ls, 3));
                    const ys = new te(ks, this.lightningMaterialBase.clone());
                    ys.material.opacity = .6, ys.userData.life = .16 + Math.random() * .12, this.scene.add(ys), this.activeLightningLines.push(ys);
                }
            }
            const ms = 2;
            for(let _ = -1; _ <= ms; _++){
                const k = _ === -1, S = k ? 0 : _ % 2 === 0 ? 1 : -1, J = .55 + Math.random() * .45, ns = .35, Z = Math.random() * Math.PI * 2, K = [], Q = Y.length / 3;
                for(let W = 0; W < Q; W++){
                    const ls = W / (Q - 1), vs = .35 + .85 * ls, rs = Math.sin(ls * Math.PI * 2 + Z) * ns * (.2 + .8 * ls), ws = g * S * J * vs + z * rs * .3, Ds = m * S * J * vs + rs * .05, xs = z * S * J * vs - g * rs * .3, ks = W * 3 + 0, ys = W * 3 + 1, zs = W * 3 + 2, $s = Y[ks], Zs = Y[ys], Qs = Y[zs];
                    k ? K.push($s + (Math.random() - .5) * .05, Zs + (Math.random() - .5) * .05, Qs + (Math.random() - .5) * .05) : K.push($s + ws + (Math.random() - .5) * .2, Zs + Ds + (Math.random() - .5) * .2, Qs + xs + (Math.random() - .5) * .2);
                }
                const ss = new Ps;
                ss.setAttribute("position", new ee(K, 3));
                const U = new te(ss, this.lightningMaterialBase.clone());
                U.material.opacity = k ? .95 : .32, U.userData.life = .22 + Math.random() * .18, this.scene.add(U), this.activeLightningLines.push(U);
            }
        }
    ...
    class ht {
        ...
        loadRecsData() {
            ...
            t[0].innerHTML = "", t[1].innerHTML = "", s.forEach((a, o, n)=>{
                s[o].forEach((l, r, d)=>{
                    s[o][r].findIndex((y)=>y.name === "Мой рекорд") < 3 ? t[o].insertAdjacentHTML("beforeend", `
        ...
        mainMenu = ()=>{
            ...
            t.addEventListener("click", (i)=>{
                ...
                i.addEventListener("click", ()=>{
                    ...
                    o.forEach((d)=>{
                        const y = d.querySelector(".rec_table_small_block:not(.hidden_screen)");
                        y && (n.push(y), y.getBoundingClientRect(), y.classList.add("anim-out"));
                    });
                    let l = 0;
                    const r = ()=>{
                        ...
                        document.querySelectorAll(".rec_table_small").forEach((y)=>{
                            const u = y.querySelector(".rec_table_small_block:not(.hidden_screen)");
                            ...
                            const y = (u)=>{
                                u.classList.remove("anim-in", "anim-play"), u.removeEventListener("transitionend", y);
                            };
                            d.forEach((u)=>u.addEventListener("transitionend", ()=>y(u), {
                                    once: !0
                                }));
                        });
                    };
    ...
    class ct {
        ...
        updateMetersFloat(s, t, i = "hor") {
            ...
            for (const u of t){
                ...
                let b = f.position?.[e] ?? 0;
                if (c._lastMeterPos == null && (c._lastMeterPos = b), i !== "vert" && c._wasLive === !1 && c.live && (c._lastMeterPos = b), c.live) {
                    const g = b - c._lastMeterPos, m = g > a ? g : 0;
                    m !== 0 && (c.score += m, c._lastMeterPos = b);
                }
                c.score === 0 && (c._lastMeterPos = b), c._wasLive = !!c.live;
            }
            this.playerPanels || (this.playerPanels = Array.from(document.querySelectorAll(".player_panel_rec_num")).slice(0, 3));
            let o = 0;
            for(let u = 0; u < 3; u++){
                const f = this.playerPanels[u], c = t[u]?.player, b = Math.max(0, Math.floor(c?.userData?.score || 0));
                o += b, f && (f.textContent = String(b).padStart(3, "0"));
            }
            const n = Math.max(0, Math.floor(o));
            if (n === qs) return;
            const l = qs, r = performance.now(), d = 50, y = (u)=>{
                const f = Math.min(1, (u - r) / d), c = 1 - Math.pow(1 - f, 4), b = Math.round(l + (n - l) * c);
                this.score = b, this.metersFloatEl && (this.metersFloatEl.textContent = String(b).padStart(3, "0")), f < 1 ? requestAnimationFrame(y) : qs = n;
            };
    ...
    class ut {
        ...
        async loadLevels(s) {
            ...
            for(let l = 0; l < this.levelsStatus[s].length; l++){
                const r = this.levelsStatus[s][l], { modifierClass: d, labelText: y, ariaState: u } = e(r), f = document.createElement("div");
                ...
                b.className = "levels_block_number", b.textContent = String(l + 1);
                const g = document.createElement("div");
                g.className = "levels_block_status";
                const m = document.createElement("span");
                m.className = `status_chip ${r === "completed" ? "status_chip--completed" : r === "available" ? "status_chip--available" : "status_chip--locked"}`, m.textContent = y, g.append(m), f.append(b, g), f.addEventListener("click", ()=>{
                    r !== "locked" && (document.querySelectorAll(".levels_block").forEach((z)=>z.classList.remove("active")), f.classList.add("active"), console.log(`Выбран уровень ${l + 1}`));
                }), f.addEventListener("keydown", (z)=>{
                    r !== "locked" && (z.key === "Enter" || z.key === " ") && (z.preventDefault(), f.click());
                }), i.append(f);
            }
        ...
        loadLevelsContest() {
            ...
            for(let o = 0; o < this.allLevels; o++){
                ...
                y.className = "levels_block_number", y.textContent = String(n);
                const u = document.createElement("div");
                u.className = "levels_block_status", l && (u.textContent = this.names[l - 1]), r.append(y, u), r.addEventListener("click", ()=>{
    ...
    console.clear();
    let Ys, mt = new Ts, fe, os, gs, $, x, j, _s, q, Ss, T, D = new pt;
    const ps = new We;
    ps.background = new is(13230580);
    const ge = tt({
        scene: ps
    }), ve = at({
        scene: ps
    }), F = new Oe(25, window.innerWidth / window.innerHeight, .1, 2e3);
    F.position.y = 2;
    const yt = 16 / 9, bt = I.degToRad(25), ft = 2 * Math.atan(Math.tan(bt / 2) * yt), Ws = Qe();
    function Xs() {
    ...
    let Ls = new Ue;
    document.body.appendChild(Ls.dom);
    Ls.dom.style.top = "0px";
    Ls.dom.style.left = "548%";
    const H = new Ve({
        antialias: !1
    });
    ...
    function we() {
        ...
        const e = I.degToRad(12), a = I.degToRad(70);
        i = I.clamp(i, e, a), F.fov = I.radToDeg(i), F.aspect = t, F.updateProjectionMatrix(), H.setSize(h, s);
    }
    ...
    async function gt() {
        ue(!0), T = new ut, await T.loadLocalData(), await T.loadLevels(0), await T.loadLevelsContest(), j = new nt, await j.loadAudio(), os = new ht(De, T.loadLevels, D, j, T), ue(!1);
    }
    await gt();
    async function vt(h) {
        ...
        Ys = new s.World(new s.Vector3(0, -9.81, 0)), fe = new s.EventQueue(!0), $ = new cs(Ys, s), Ss = new ct(F, T), gs = new rt(ps, F, H, q, Ws, j), x = new ot(ps, j, $, H, F, Ws, q, gs, De, T, D, ge, ve, Ss, os);
        for(let t = 0; t < h; t++)x.players.push(new it(T, ps, j, x, q, F, D));
        _s = new lt(x, Ws, H, F, q, j), _s.addKeyListeners();
    }
    async function wt() {
        await gs.loadWorld(), j.musicOn && j.backAudio.play(), j.musicOn && j.oceanAudio.play();
    }
    async function Dt(h) {
        await x.loadPlayers(), await x.createLevel(h), await x.loadEnvironments(), x.objs.grassPlanes.data.length > 0 && x.objs.grassPlanes.data.forEach((s, t)=>{
            x.objs.grassPlanes.data[t].userData.collide.setCollisionGroups(Fs([
                0
            ], [
                1
            ]));
        }), x.players.length > 0 && x.players.forEach((s, t)=>{
            x.players[t].player.userData.collider.setCollisionGroups(Fs([
                1
            ], [
                0,
                1
    ...
    async function De(h, s, t = !1) {
        xt(), os.toggleLoader(!0), q = new dt, await vt(h), x.gameNum = s, await wt(), await Dt(t), q.gameDir === "hor" ? Ss.loadRecsToHud(0, x.players.length - 1) : Ss.loadRecsToHud(1, x.players.length - 1), q.dataLoaded = !0, D.gameStarting = !0, T.gameInit = !0, setTimeout(()=>{
            os.toggleLoader(!1);
        }, 200);
    }
    function xt() {
        F.position.y = 2, F.position.x = 0, H.toneMappingExposure = 1.05, _s?.removedKeyListeners(), gs = null, $ = null, x = null, _s = null, q = null, Ss = null;
    }
    function Pt() {
        if (D.gameStarting && document.querySelector(".menu_in_game").classList.contains("hidden_screen") && q.dataLoaded && x.showScreen("menu_in_game"), T.gameInit && D.gameStarting && !x.levelsMode && document.querySelector(".hud").classList.contains("hidden_screen") && q.dataLoaded ? (os.showScreen("hud"), os.hideScreen("level_hud_wrap")) : !T.gameInit && !document.querySelector(".hud").classList.contains("hidden_screen") && (os.hideScreen("hud"), os.showScreen("level_hud_wrap")), T.gameInit && D.gameStarting && x.levelsMode && !document.querySelector(".player_panel_rec").classList.contains("hidden_screen") ? document.querySelectorAll(".player_panel_rec").forEach((h, s, t)=>{
            h.classList.add("hidden_screen");
        }) : T.gameInit && D.gameStarting && !x.levelsMode && document.querySelector(".player_panel_rec").classList.contains("hidden_screen") && document.querySelectorAll(".player_panel_rec").forEach((h, s, t)=>{
            h.classList.remove("hidden_screen");
        }), D.gameStarting && (ge.update(Rs), ve.update(Rs)), q.dataLoaded && D.gameStarting) {
            Ls.dom.style.left = "48%", x.players.forEach((h, s, t)=>{
                h.playerMove();
            }), gs.updateLighting(), x.levelAnimate(F), x.cameraMove(F), Ls.update();
            ...
            $.updateInstancedTransforms(), Ys.step(fe), D.gameStarting && H.render(ps, F);
        }
    }
    let Os = 0;
    const Rs = 1 / 60, pe = .1;
    H.setAnimationLoop(()=>{
        if (q != null) {
            let h = mt.getDelta();
            for(h > pe && (h = pe), Os += h; Os >= Rs;)Pt(), Os -= Rs;
        }
    });
    function ue(h) {
    ...
    document.addEventListener("visibilitychange", function() {
        document.visibilityState === "visible" ? (!D.pause && !D.showGamePopup && (D.gameStarting = !0, j.togglePauseAll(!D.gameStarting)), D.visible = !0) : (!D.pause && !D.showGamePopup ? (D.gameStarting = !1, j.togglePauseAll(!D.gameStarting)) : D.pause || j.togglePauseAll(!D.gameStarting), D.visible = !1);
    });
    document.querySelector(".pause_btn").addEventListener("click", ()=>{
        !D.pause && D.gameStarting && (D.pause = !D.pause, D.pause && (x.showPopupInGame(), D.gameStarting = !1, j.togglePauseAll(!D.gameStarting), x.showScreen("popup_game_btn_close")));
    });
    document.querySelector(".popup_game_btn_close").addEventListener("click", ()=>{
        (D.pause || D.gameStarting) && (D.pause = !D.pause, D.gameStarting = !0, j.togglePauseAll(!D.gameStarting), gs.rain && !j.rainAudio.isPlaying && j.rainAudio.play(), j.oceanAudio.isPlaying || j.oceanAudio.play(), x.hideScreen("popup_in_game"), x.hideScreen("popup_game_btn_close"));
    });
    ...
    function Ct() {
        ...
        const n = ()=>{
            ...
            if (!s || !t || !i) return;
            const c = s.clientHeight, b = s.scrollHeight, g = s.scrollTop;
            if (b <= c + 1) {
                t.classList.remove("visible");
                return;
            }
            t.classList.add("visible");
            const m = t.getBoundingClientRect().height, N = Math.max(c / b * m, 24), G = b - c, v = m - N, w = G > 0 ? g / G * v : 0;
            i.style.height = `${N}px`, i.style.top = `${w}px`;
        }, d = (c)=>{
            !s || !i || (e = !0, a = c.touches ? c.touches[0].clientY : c.clientY, o = s.scrollTop, document.body.style.userSelect = "none", c.preventDefault());
        }, y = (c)=>{
            if (!e || !s || !i || !t) return;
            const g = (c.touches ? c.touches[0].clientY : c.clientY) - a, m = t.getBoundingClientRect().height, z = s.clientHeight, N = s.scrollHeight, G = Math.max(1, m - i.offsetHeight), v = (N - z) / G;
            s.scrollTop = o + g * v;
        }, u = ()=>{
