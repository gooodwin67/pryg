import { M as D, B as w, a as H, b as k, G as z, V as c, T as N, R as P, c as Y, A as $, d as Z, P as C, e as Q, f as ee, H as te, D as se, O as ie, g as ae, h as oe, i as G, W as re, S as ne, j as le, k as he, C as V, l as de, m as _, n as pe, o as ce, p as ue, q as F, r as ye, s as me, t as fe, u as ge, v as we, w as ve } from "./three-CInlo0fg.js";
(async ()=>{
    (function() {
    ...
    const De = "modulepreload", Me = function(n, e) {
        return new URL(n, e).href;
    }, I = {}, Le = function(e, t, s) {
        let i = Promise.resolve();
        if (t && t.length > 0) {
            let l = function(h) {
                return Promise.all(h.map((m)=>Promise.resolve(m).then((x)=>({
                            status: "fulfilled",
                            value: x
                        }), (x)=>({
                            status: "rejected",
            ...
            const o = document.getElementsByTagName("link"), r = document.querySelector("meta[property=csp-nonce]"), d = r?.nonce || r?.getAttribute("nonce");
            i = l(t.map((h)=>{
                if (h = Me(h, s), h in I) return;
                I[h] = !0;
                const m = h.endsWith(".css"), x = m ? '[rel="stylesheet"]' : "";
                if (!!s) for(let b = o.length - 1; b >= 0; b--){
                    const S = o[b];
                    if (S.href === h && (!m || S.rel === "stylesheet")) return;
                }
                else if (document.querySelector(`link[href="${h}"]${x}`)) return;
                const f = document.createElement("link");
                if (f.rel = m ? "stylesheet" : De, m || (f.as = "script"), f.crossOrigin = "", f.href = h, d && f.setAttribute("nonce", d), document.head.appendChild(f), m) return new Promise((b, S)=>{
                    f.addEventListener("load", b), f.addEventListener("error", ()=>S(new Error(`Unable to preload CSS for ${h}`)));
                });
            }));
        }
        function a(o) {
        ...
        return i.then((o)=>{
            for (const r of o || [])r.status === "rejected" && a(r.reason);
            return e().catch(a);
        });
    };
    function M(n, e) {
    ...
    function g(n, e) {
        n.geometry.computeBoundingBox(), e.forEach(function(i, a, o) {
            i.geometry.computeBoundingBox();
        }), n.updateMatrixWorld(), e.forEach(function(i, a, o) {
            i.updateMatrixWorld();
        });
        let t = n.geometry.boundingBox.clone();
        t.applyMatrix4(n.matrixWorld);
        var s = !1;
        for(let i = e.length - 1; i > -1; i--)if (e[i].userData.id == null || e[i].userData.id != n.uuid) {
    ...
    function q(n) {
        const e = n.collisionGroups(), t = e >>> 16 & 65535, s = e & 65535;
        function i(a) {
            const o = [];
            for(let r = 0; r < 16; r++)a & 1 << r && o.push(r);
            return o;
        }
        return [
            i(t),
    ...
    class be {
        ...
        playerMove() {
            if (g(this.player, this.levelClass.sensorPlanes)) {
                const [e, t] = q(this.player.userData.collider);
            ...
            if (g(this.player, this.levelClass.topPlanes) ? this.player.userData.onGround = !0 : this.player.userData.onGround = !1, this.playerModel.position.x = this.player.position.x, this.playerModel.position.y = this.player.position.y - this.playerHeight / 2, this.playerModel.position.z = this.player.position.z, this.playerOut.position.copy(this.player.position), this.playerOut.rotation.copy(this.player.rotation), g(this.player, this.levelClass.boostHatMeshes) && (this.player.userData.canFly && (this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].position.copy(new c(this.player.userData.head.getWorldPosition(new c).x - .05, this.player.userData.head.getWorldPosition(new c).y + .15, this.player.userData.head.getWorldPosition(new c).z + .1)), this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].children[0].children[1].rotation.y += .4), this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].userData.fly || (this.player.userData.numHatBoost = this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes)), this.player.userData.canFly = !0, this.player.userData.hatBoost = 2), this.levelClass.boostHatModels[this.levelClass.boostHatMeshes.indexOf(g(this.player, this.levelClass.boostHatMeshes))].userData.fly = !0), this.playerModel.position.y < -2 && (this.player.userData.live = !1), !this.player.userData.live) this.player.userData.body.setTranslation(this.player.userData.startPos), this.player.userData.readyJump = !1, this.player.userData.canFly = !1, this.player.userData.hatBoost = 0, this.player.userData.numHatBoost = 0, this.player.userData.jumping = !1, this.player.userData.playerPowerJump = 1, this.player.userData.onGround = !1, this.player.userData.body.setLinvel({
                x: 0,
                y: 0,
                z: 0
            }, !0);
            else {
                const e = this.player.userData.readyJump ? Math.PI / 2 : 0, t = this.player.userData.readyJump ? -Math.PI / 2 : 0, s = this.player.userData.body.linvel().y < -2 ? Math.PI / 2 : 0, i = this.player.userData.body.linvel().y < -3 ? -Math.PI / 2 : 0, a = this.player.userData.readyJump ? Math.PI / 8 : 0, o = this.player.userData.body.linvel().y < -.4 ? Math.PI / 7 : 0, r = this.player.userData.body.linvel().y > .4 ? Math.PI / -5.9 : 0, d = this.player.userData.readyJump ? .75 : 1.18, l = this.player.userData.readyJump ? .75 : .15;
                this.rightHand.rotation.z = this.lerp(this.rightHand.rotation.z, e, .1), this.leftHand.rotation.z = this.lerp(this.leftHand.rotation.z, t, .1), this.rightHand.rotation.z = this.lerp(this.rightHand.rotation.z, s, .1), this.leftHand.rotation.z = this.lerp(this.leftHand.rotation.z, i, .1), this.head.rotation.x = this.lerp(this.head.rotation.x, a, .1), this.head.position.y = this.lerp(this.head.position.y, d, .1), this.head.position.z = this.lerp(this.head.position.z, l, .1), this.head.rotation.z = this.lerp(this.head.rotation.z, o, .1), this.head.rotation.z = this.lerp(this.head.rotation.z, r, .1);
                const h = this.player.userData.body.linvel().y < -3 ? Math.PI / 1.3 : Math.PI;
                this.playerModel.rotation.y = this.lerp(this.playerModel.rotation.y, h, .1);
    ...
    class Ce {
        constructor(e, t, s, i, a, o){
            this.scene = e, this.audioClass = t, this.physicsClass = s, this.renderer = i, this.camera = a, this.isMobile = o, this.planeWidth = 4, this.planeHeight = 10, this.geometryPlane = new w(this.planeWidth * 1.5, this.planeHeight, 1), this.materialPlane = new H({
                color: 52224
            }), this.plane = new D(this.geometryPlane, this.materialPlane), this.plane.receiveShadow = !0, this.plane.castShadow = !0, this.plane.position.y = -this.planeHeight / 2, this.plane.userData.name = "plane", this.planes = [], this.planeTop = new D(new w(this.geometryPlane.parameters.width, .4, 1.2), new k({
                color: 13421568,
                transparent: !0,
                opacity: 0
            })), this.planeTop.position.y = this.plane.position.y + this.planeHeight / 2 + .1, this.planeTop.userData.direction = 1, this.topPlanes = [], this.planeGrass = new D(new w(this.geometryPlane.parameters.width, .5, 1.2), new H({
                color: 52224,
                transparent: !0,
                opacity: 1
            })), this.planeGrass.position.y = this.plane.position.y + this.planeHeight / 2 + .1, this.planeGrass.castShadow = !0, this.planeGrass.receiveShadow = !0, this.planeGrass.userData.name = "tops", this.planeGrass.userData.direction = 1, this.grassPlanes = [], this.planeSensor = new D(new w(this.geometryPlane.parameters.width, .4, 1.2), new H({
                color: 65280,
                transparent: !0,
                opacity: 0
            })), this.planeSensor.position.y = this.plane.position.y + this.planeHeight / 2 + .1, this.planeSensor.userData.name = "top_sensor", this.sensorPlanes = [], this.boostHatModel, this.boostHatMesh, this.boostHatPropeller, this.boostHatModels = [], this.boostHatMeshes = [], this.players = [], this.cloudModel, this.clouds = [], this.backModel, this.backModels = [], this.leftEdge = new c(-1, 0, 0), this.rightEdge = new c(1, 0, 0), this.leftEdge.unproject(a), this.rightEdge.unproject(a), this.bounds, this.getHorizontalWorldBounds(), this.gameNum = 1, this.gameDir = "vert";
        }
        async loadTexture() {
            const e = new N;
            e.load("textures/povrezdennaa-tekstura-ili-fon.jpg", (t)=>{
                ...
                t.wrapS = P, t.wrapT = P, t.repeat.set(this.planeWidth / 4, this.planeHeight / 4), this.plane.material = s;
            }, void 0, function(t) {
                ...
                t.wrapS = P, t.wrapT = P, t.repeat.set(this.planeWidth / 1, this.planeHeight / 8), this.planeGrass.material = s;
            }, void 0, function(t) {
                console.error("An error happened.");
            });
        }
        async createLevel() {
            switch(await this.loadTexture(), await this.loadBoostsModel(), await this.loadEnvironmentModel(), this.cameraMove(this.camera), this.getHorizontalWorldBounds(), this.gameNum){
                ...
                case 2:
                    ...
                    let e = -2.5;
                    for(let s = 0; s < 50; s++){
                        let i = this.plane.clone(), a = this.planeTop.clone(), o = this.planeGrass.clone(), r = M(this.planeWidth / 8, this.planeWidth), d = M(2, 3), l = e + r / 2 + d, h = M(-1, 1) - this.planeHeight / 2;
                        s > 0 && (this.changeMeshWidth(i, r), this.changeMeshWidth(a, r + .3), this.changeMeshWidth(o, r + .3)), s > 0 ? (i.position.x = l, i.position.y = h + this.planeHeight / 6, a.position.x = l, a.position.y = h + this.planeHeight / 1.5 + .2, o.position.x = l, o.position.y = h + this.planeHeight / 1.5) : a.position.y = o.position.y + .2, this.planes.push(i), this.topPlanes.push(a), this.grassPlanes.push(o), e = l + r / 2;
                    }
                    break;
                case 3:
                case 4:
                    ...
                    for(let s = 0; s < 50; s++){
                        ...
                        a.userData.speed = M(4, 10) / 100;
                        let r = M(this.bounds.rightX / 2, this.bounds.rightX / 12), d = M(3, 4), l = t + d;
                        i.position.y = l, a.position.y = l, o.position.y = l, s > 0 ? (this.changeMeshWidth(i, r + .3), this.changeMeshWidth(a, r + .3), this.changeMeshWidth(o, r + .5)) : (this.changeMeshWidth(i, 10), this.changeMeshWidth(a, 10), this.changeMeshWidth(o, 10)), this.topPlanes.push(i), this.grassPlanes.push(a), this.sensorPlanes.push(o), t = l;
                    }
                    break;
            }
        }
        getHorizontalWorldBounds(e = 0) {
            const t = new c(-1, 0, .5), s = new c(1, 0, .5);
            if (t.unproject(this.camera), s.unproject(this.camera), this.camera.isPerspectiveCamera) {
                const i = this.camera.position, a = t.clone().sub(i).normalize(), o = s.clone().sub(i).normalize(), r = (e - i.z) / a.z, d = i.clone().add(a.multiplyScalar(r)), l = i.clone().add(o.multiplyScalar(r));
                this.bounds = {
                    leftX: d.x,
                    rightX: l.x
                };
        ...
        animateTops() {
            if (this.gameDir == "vert") for(let e = 0; e < this.grassPlanes.length; e++){
                ...
                const r = t.userData.direction * a, d = o.x + r;
        ...
        async loadEnvironmentModel() {
            await new z().loadAsync("models/environment/clouds.glb").then((s)=>{
                const i = s.scene;
                this.cloudModel = i, this.cloudModel.children.forEach((a, o, r)=>{
    ...
    class Pe {
        ...
        addPhysicsToObject(e) {
            ...
            const o = new Y().setFromObject(e).getSize(new c);
            if (e.rotation.copy(i), e.userData.name.includes("player")) {
                ...
                e.userData.collider = d, e.userData.handle = r.handle, this.playersHandles.push(r.handle), this.dynamicBodies.push([
                    e,
                    t,
                    e.id
                ]);
                ...
                let r = this.world.createCollider(s, t);
                this.allWallBodyCollision.push(r), e.userData.handle = t.handle, this.dynamicBodies.push([
                    e,
                    t,
                    e.id
                ]);
                ...
                let r = this.world.createCollider(s, t);
                e.userData.body = t, e.userData.collide = r, this.allWallBodyCollision.push(r), e.userData.handle = t.handle, this.dynamicBodies.push([
                    e,
                    t,
                    e.id
    ...
    class Se {
        ...
        async loadAudio() {
            ...
            await t.loadAsync("audio/ready-jump.wav").then((s)=>{
                this.readyJumpAudio = new C(e), this.readyJumpAudio.setBuffer(s), this.readyJumpAudio.setLoop(!1), this.readyJumpAudio.setRefDistance(400), this.readyJumpAudio.setVolume(30), this.readyJumpAudio.setPlaybackRate(2);
            }).catch((s)=>{
                console.error("Ошибка при загрузке аудио:", s);
            }), await t.loadAsync("audio/quack1.mp3").then((s)=>{
                this.jumpAudio = new C(e), this.jumpAudio.setBuffer(s), this.jumpAudio.setLoop(!1), this.jumpAudio.setRefDistance(400), this.jumpAudio.setVolume(.3), this.quacks.push(this.jumpAudio);
            }).catch((s)=>{
                console.error("Ошибка при загрузке аудио:", s);
            }), await t.loadAsync("audio/quack2.mp3").then((s)=>{
                this.jumpAudio2 = new C(e), this.jumpAudio2.setBuffer(s), this.jumpAudio2.setLoop(!1), this.jumpAudio2.setRefDistance(400), this.jumpAudio2.setVolume(.3), this.quacks.push(this.jumpAudio2);
            }).catch((s)=>{
                console.error("Ошибка при загрузке аудио:", s);
            }), await t.loadAsync("audio/quack3.mp3").then((s)=>{
                this.jumpAudio3 = new C(e), this.jumpAudio3.setBuffer(s), this.jumpAudio3.setLoop(!1), this.jumpAudio3.setRefDistance(400), this.jumpAudio3.setVolume(.3), this.quacks.push(this.jumpAudio3);
            }).catch((s)=>{
                console.error("Ошибка при загрузке аудио:", s);
            }), await t.loadAsync("audio/quack5.mp3").then((s)=>{
                this.jumpAudio4 = new C(e), this.jumpAudio4.setBuffer(s), this.jumpAudio4.setLoop(!1), this.jumpAudio4.setRefDistance(400), this.jumpAudio4.setVolume(.3), this.quacks.push(this.jumpAudio4);
            }).catch((s)=>{
                console.error("Ошибка при загрузке аудио:", s);
            });
        }
    ...
    class Ae {
        constructor(e, t, s, i){
            this.levelClass = e, this.isMobile = t, this.renderer = s, this.camera = i, this.camera = i, this.mouse = new c, this.raycaster = new Q, this.addKeyListeners();
        }
        addKeyListeners() {
    ...
    class He {
        constructor(e, t, s, i){
            this.scene = e, this.camera = t, this.levelClass = s, this.renderer = i, this.ambientLight = new ee(11184810, 1), this.hemiLight = new te(16777215, 16777215, 2), this.hemiLight.groundColor.setHSL(.095, 1, .75), this.hemiLight.position.set(0, 10, 0), this.dirLight = new se(16777215, 2), this.dirLight.position.set(0, 5, 5), this.dirLight.castShadow = !0, this.dirLight.shadow.camera.far = 100, this.topLight = 1e3, this.targetObject = new ie, this.dirLight.target = this.targetObject, this.helper = new ae(this.dirLight, 3), this.water;
            const a = 50, o = 50, r = 2;
            this.pointLight = new oe(16777215, r, a, o), this.pointLight.position.set(30, 40, 0), this.pointLight.lookAt(0, 0, 0);
        }
        async loadWaterSky() {
            this.waterGeometry = new G(1e4, 300), this.water = new re(this.waterGeometry, {
                textureWidth: 500,
                textureHeight: 500,
                waterNormals: new N().load("textures/waternormals.jpg", function(l) {
                    l.wrapS = l.wrapT = P;
                }),
                sunDirection: new c,
                sunColor: 16777215,
            ...
            }), this.water.rotation.x = -Math.PI / 2, this.water.position.y = 0, this.sun = new c, this.sky = new ne, this.sky.scale.setScalar(1e4), this.scene.add(this.sky);
            ...
            e.turbidity.value = 1, e.rayleigh.value = 3, e.mieCoefficient.value = 5e-4, e.mieDirectionalG.value = .8, this.parameters = {
                ...
                azimuth: 170,
                top: !1
            }, this.blackSky = new D(new G(1e4, 1e4), new le({
                color: 526362,
                side: he,
                transparent: !0,
                opacity: 0
            ...
            for(let l = 0; l < t; l++){
                ...
                const h = new V().setHSL(.5 + Math.random() * .1, .6 + Math.random() * .3, .85 + Math.random() * .15);
                a[3 * l] = h.r, a[3 * l + 1] = h.g, a[3 * l + 2] = h.b;
            }
            const o = new de;
            o.setAttribute("position", new _(s, 3)), o.setAttribute("size", new _(i, 1)), o.setAttribute("color", new _(a, 3));
            const r = `
...
`;
            this.materialStars = new pe({
                ...
                depthWrite: !1,
                blending: ce
            }), this.stars = new ue(o, this.materialStars), this.stars.layers.set(1), this.scene.add(this.stars), this.camera.layers.enable(1);
        }
        updateSky() {
            this.stars.position.x = this.camera.position.x;
            const e = F.degToRad(90 - this.parameters.elevation), t = F.degToRad(this.parameters.azimuth);
            if (this.sun.setFromSphericalCoords(1, e, t), this.sky.material.uniforms.sunPosition.value.copy(this.sun), this.water.material.uniforms.sunDirection.value.copy(this.sun).normalize(), this.levelClass.gameDir == "hor" && (this.sun.y < -.07 && this.materialStars.uniforms.opacity.value < 1 ? (this.materialStars.uniforms.opacity.value += .001, this.blackSky.material.opacity < .8 && (this.blackSky.material.opacity += .001)) : this.sun.y > -.07 && this.materialStars.uniforms.opacity.value > 0 && (this.materialStars.uniforms.opacity.value -= .001, this.blackSky.material.opacity -= .001), this.parameters.elevation < -8 ? this.parameters.top = !0 : this.parameters.elevation > 6 && (this.parameters.top = !1), this.parameters.top ? (this.parameters.elevation += .003, this.dirLight.intensity += 3e-4, this.dirLight.intensity = Math.max(.5, Math.min(2, this.dirLight.intensity)), this.hemiLight.intensity += 3e-4, this.hemiLight.intensity = Math.max(.5, Math.min(2, this.hemiLight.intensity))) : (this.parameters.elevation -= .003, this.dirLight.intensity -= 3e-4, this.dirLight.intensity = Math.max(.5, Math.min(2, this.dirLight.intensity)), this.hemiLight.intensity -= 3e-4, this.hemiLight.intensity = Math.max(.5, Math.min(2, this.hemiLight.intensity))), console.log(this.dirLight.intensity)), this.levelClass.gameDir == "vert") {
    ...
    console.clear();
    let B, U = !1, A = 0, J = 1 / 60, Ee = new ve, j, W, T, v, p, E;
    const L = new ye;
    L.background = new V(13230580);
    const u = new me(25, window.innerWidth / window.innerHeight, .1, 2e3);
    u.position.z = 7;
    u.position.y = 2;
    const K = xe();
    let X = new fe;
    ...
    async function _e(n) {
        const e = await Le(()=>import("./@dimforge-BObwuXYQ.js").then(async (m)=>{
                await m.__tla;
                return m;
            }), [], import.meta.url);
        B = new e.World(new e.Vector3(0, -9.81, 0)), j = new e.EventQueue(!0), v = new Pe(B, e), E = new Se, p = new Ce(L, E, v, y, u, K), T = new He(L, u, p, y);
        for(let t = 0; t < n; t++)p.players.push(new be(L, E, p));
        new Ae(p, K, y, u);
    }
    async function ze() {
        await T.loadWorld(), await E.loadAudio();
    }
    async function Be() {
        await p.createLevel(), await p.loadEnvironments(), await p.loadPlayers(), p.grassPlanes.forEach((n, e)=>{
            p.grassPlanes[e].userData.collide.setCollisionGroups(R([
                0
            ], [
                1
            ]));
        }), p.players.forEach((n, e)=>{
            p.players[e].player.userData.collider.setCollisionGroups(R([
                1
            ], [
                0,
                1
    ...
    async function We(n, e) {
        W.toggleLoader(!0), await _e(n), p.gameNum = e, await ze(), await Be(), W.toggleLoader(!1), U = !0;
    }
    W = new ke(We);
    function Te() {
        if (U) {
            p.players.forEach((n, e, t)=>{
                n.playerMove();
            }), T.updateLighting(), p.levelAnimate(u), p.cameraMove(u), X.update();
            for(let n = 0, e = v.dynamicBodies.length; n < e; n++)v.dynamicBodies[n][0].position.copy(v.dynamicBodies[n][1].translation()), v.dynamicBodies[n][0].quaternion.copy(v.dynamicBodies[n][1].rotation());
            B.step(j), y.render(L, u);
        }
    }
    y.setAnimationLoop(()=>{
        A += Ee.getDelta(), A > J && (Te(), y.render(L, u), A = A % J);
    });
})();
